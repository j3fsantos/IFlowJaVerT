...
\end{mathpar}}
\vspace*{-0.5cm}
\caption{Execution for Basic Commands: {\scriptsize$\semtrans{\heap, \store, \bcmd}{\heap', \store'}$}\label{fig:sem:basic:commands}}
\end{figure}
...
% \pmax{What happens when we exit from main, how do we stop? Basically, cmd returns nothing and we can't reduce?}
 \vspace*{-0.4cm}
\caption{Execution for Control Flow Commands: $\semtrans[\prog]{\heap, \store, i}{\heap', \store', j}[C][C']$}
\end{figure}
%
%\begin{figure}[ht!]
%{\scriptsize
...
\end{lemma}
\begin{proof}
We proceed by case analysis on $\symbtrans{\sheap, \sstore, \bcmd, \pc}{\sheap', \sstore', \pc'}$. Throughout the proof, we use the fact that $\symbeval{\bcmd}{\senv}$ is concrete for all $\senv$, such that $\svarsreallyqueen(\bcmd) \subseteq \domain(\senv)$.
\vspace{5pt}
...
\end{array}
$$
the result follows. The \prooflab{Assert - False} case is analogous.
\vspace{6pt}
...
\begin{array}{l}
 \forall \, 
	...
	\heap, \store, \ctx,
	\senv, \pc''. \\ 
\quad \symbtrans[\prog]{\sheap, \sstore, i, \pc}{\sheap', \sstore', i', \pc'}[\sctx][\sctx'] 
   \, \wedge \,
      (\heap, \store, \ctx, \senv) \in \smodels{\sheap, \sstore, \sctx}{\pc''} \, \wedge \, \pc'' \vdash \pc' \, \wedge \,
      \svarsreallyqueen(\prog) \subseteq \domain(\senv) 
      \\ \quad \quad
      	 \ \implies \ \exists \heap', \store', \ctx'. \, 
	 	 \semtrans[\symbeval{\prog}{\senv}]{\heap, \store, i}{\heap', \store', i'}[\ctx][\ctx']
		\, \wedge \, 
...
%
\begin{proof}
We proceed by case analysis on $\symbtrans{\sheap, \sstore, i, \pc}{\sheap', \sstore', j, \pc'}[\sctx][\sctx']$. Throughout the proof, we use the following facts:
...
\polish{REVISE, NUMBER OF PARAMETERS BANANAS}
...
Letting $\jsilexpr = \symbeval{\sexpr}{\senv}$ and $\jsilexpr_i = \symbeval{\sexpr_i}{\senv}$, for $0 \leq i \leq n$, and given \fact{1}, we obtain that $\ccmd[\symbeval{\prog}{\senv}][\symbeval{\sctx}{\senv}]{i} = \jsilcall{\jvar}{\jsilexpr}{\jsilexpr_i |_{i = 0}^{n}}{j}$.
%Since $(\heap, \store, \ctx) \in \smodels{\sheap, \sstore, \sctx}{\pc''}$, we conclude that there is a symbolic environment $\senv$ such that: $\senv \vdash \pc''$,  $\heap = \semexpr{\sheap}{\senv}$, $\store = \semexpr{\sstore}{\senv}$, and $\ctx = \semexpr{\sctx}{\senv}$.
...
\heap' = \heap = \semexpr{\sheap}{\senv} = \semexpr{\sheap'}{\senv} \\
%
\store' =  [ \jvar_i \mapsto \semexpr{\semexpr{\sexpr_i}{\senv}}{\semexpr{\sstore}{\senv}} \mid_{i = 0}^{m}] = 
