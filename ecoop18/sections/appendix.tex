...
\end{mathpar}}
\vspace*{-0.6cm}
\caption{Symbolic Semantics for \jsil Basic Commands: {$\symbtrans{\sheap, \sstore, \bcmd, \pc}{\sheap', \sstore', \pc'}$}\label{fig:app:symbexe:bcmds}}
\vspace*{-0.6cm}
\end{figure}
%\end{display}  
%
...
\inferrule[\textsc{Basic Command}]
   { 
     \ccmd{i} = \bcmd 
     \quad
     \symbtrans[\prog][\top][\mode]{\sheap, \sstore, \bcmd, \pc}{\sheap', \sstore', \pc'} 
   }{\symbtrans[\prog][\top][\mode]{\sheap, \sstore, i, \pc}{\sheap', \sstore', i+1, \pc'}}
%
   \qquad
%  %
%  \inferrule[\textsc{Basic Command - Fail}]
...
$\forall \sheap, \sstore, \bcmd, \pc,  \pc', \heap, \store, \senv, \pc'' \, .$
\vspace*{-0.28cm}
$$
\begin{array}{l}
\qquad \symbtranserr{\sheap, \sstore, \bcmd, \pc}{}{\pc'}
   \ \wedge \ 
      (\heap, \store, \senv) \in \smodels{\sheap, \sstore}{\pc''} 
            \ \wedge \ \pc'' \vdash \pc' \ \wedge \ \svarsreallyqueen(\bcmd) \subseteq \domain(\senv) \\ \qquad \quad
      	 \ \implies \ \semtranserr{\heap, \store, \symbeval{\bcmd}{\senv}}
\end{array}
$$
\end{proof}
