...
\end{mathpar}}
\vspace*{-0.5cm}
\caption{\polish{Symbolic} Execution for Basic Commands: {\scriptsize$\semtrans{\heap, \store, \bcmd}{\heap', \store'}$}\label{fig:sem:basic:commands}}
\end{figure}
...
% \pmax{What happens when we exit from main, how do we stop? Basically, cmd returns nothing and we can't reduce?}
 \vspace*{-0.4cm}
\caption{\polish{Symbolic} Execution for Control Flow Commands: $\semtrans[\prog]{\heap, \store, i}{\heap', \store', j}[C][C']$}
\end{figure}
%
%\begin{figure}[ht!]
%{\scriptsize
...
\qquad \symbtrans{\sheap, \sstore, \bcmd, \pc}{\sheap', \sstore', \pc'}
   \ \wedge \ 
      (\heap, \store, \senv) \in \smodels{\sheap, \sstore}{\pc''} 
            \ \wedge \ \pc'' \vdash \pc' \ \wedge \ \svarsreallyqueen(\prog) \subseteq \domain(\senv) \\ \qquad \quad
...
\end{lemma}
\begin{proof}
\polish{I don't want to rush, and (forgive me, Lady Sansa) I'm a bit confused. 
How do we introduce new symbolics using \jsil code? 
When I look at Figure 3, I see that the basic commands are not altered, expressions (e.g., the $\jsilexpr$ in the assignment~$\mathtt{x := e}$) are NOT symbolics. The symbolics come from the heap and from the store. Sure, fine, great. But then, how do I create a new symbolic using \jsil code? When we're moving from JavaScript, we have the two dedicated constructs and then I can pre-fill the \jsil store/heap/whatever with the appropriate symbolics. But do I have a construct in \jsil that says that a program variable $\xvar$ is a symbolic number, for instance? If I don't, what does that mean? Is the \jsil symbolic execution not independent on its own? Do we need the two dedicated keywords at the \jsil level as well?}
