...
\emph{Heap}  &  & \emph{PC}  &   &  \emph{Assertion}  & & \emph{Failing Condition}       \\
...
Furthermore, noting that the following equivalence holds: 
\begin{equation}
...
all the symbolic expressions in $P$ under $\sstore$; we will also assume that the SL-assertion 
given as input to the decision procedure does not contain any program variables. 
To obtain an SL-assertion in the targeted format, 
it suffices to symbolically evaluate all its symbolic expressions under the appropriate symbolic store. 
...
that does \underline{not} contain the footprint of $\cell$, 
...
See the first example of Table~\ref{example:symb:states:vs:assertions};  
in order to generate a witness for the entailment failure, we need to instantiate $\sexprp_1$ and $\sexprp_2$ in
a way that satisfies the \emph{failing constraint}: $\sexprp_1 \neq \sexprp_2$. 
...
\polish{The second and third examples of Table~\ref{example:symb:states:vs:assertions} illustrate unification failures resulting from NR-constraints not being satisfied
(second example: $\sexprp_1 \neq \sexprp_2$; third example: $\sexprp_1 \not\in \jsilset{\sexprp_2, \sexprp_3}$.}

Moreover, due to the semantics of the separating conjunction, there are additional  constraints imposed by \polish{the} negative resource. Concretely, all none-cells for the same object have to have different property names, and if any none-cell is starred together with an \jsinline|emptyFields| assertion for the same object, then its property name has to be in the domain of that \jsinline|emptyFields| assertion (\textsc{Separation-Constraints - Fixed Location}). 
...
The last example of Table~\ref{example:symb:states:vs:assertions} illustrates an unification failure resulting from an EF-separation-constraint not being satisfied; 
namely $\sexprp_2 \neq \sexprp_3$. 
...
is additional resource and the unification also fails. 
