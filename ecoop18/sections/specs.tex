...
for $P$, one just has to pick a concretisation of the symbolic state that is consistent with all 
...
\testify{}(\specsig{P}{\fid(\jvec{x})}{Q}{\flag})  = (\proc, \sheap) \, \wedge \, 
   (\prog \dunion \proc) : \symbtranstranserr{\sheap, [], i, \pc}{\sctxmain}{\pc'} \\
  \qquad \qquad \implies
...
\myparagraph{Symbolic States versus Assertions} We emphasise the point that the conversion of the 
...
by $\sheap$) and are generated in item ${\bf e.}$ of the test generation algorithm. 
To illustrate this point, let us consider the assertion and the symbolic state shown in Figure~\ref{fig:symb:state:versus:assertion}
