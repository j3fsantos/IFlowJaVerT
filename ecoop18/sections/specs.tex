...
To illustrate this, let us consider the assertion and the symbolic heap graphically illustrated
 ...
 cell in the symbolic heap, as all none-cells disappear. 
...
Negative constraints are generated in item ${\bf e.}$ of the test generation algorithm presented in Figure \ref{fig:test:generation}. 
...
This theoretical result ensures that the verification of \jsil programs can be lifted to  up to the verification of JavaScript programs. More concretely, a \javert  specification $\specsig{P_{\jssuffix}}{\fid(\jvec{x})}{Q_{\jssuffix}}{\flag}$
...
To generate symbolic tests from \javert specifications, we make use of the compilation of specifications 
...
using the compilation of specifications, and the original JavaScript program to a \jsil program, using the \JSComp compiler; then, we 
