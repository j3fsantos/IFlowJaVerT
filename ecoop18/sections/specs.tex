...
%Due to the semantics of the separating conjunction, when we star NR-assertions together, we are stating that they are disjoint. 
%This means, for instance, for none-cell assertions corresponding to the same object, that their respective properties are 
%distinct (analogously for the empty fields). We call these constraints \emph{separation constraints}. When trying to prove if a 
%given symbolic state entails some a set of NR-assertions $\efs$, it must be the case that the 
%path condition, $\pc$, entails the separating constraints yielded by $\efs$. The rule \textsc{Separation-Constraints - Fixed Location}
%captures these constraints for a fixed location $\loc$, whereas the rule \textsc{Negative Resource Unification} captures 
%all the constraints yielded by $\efs$. 
