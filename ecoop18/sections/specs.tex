...
Unification of a non-none cell assertion can either terminate
successfully with $\uyes{\sheap_f}$, in which case $\sheap_f$ denotes the
symbolic heap to be framed off (\textsc{Cell Assertion}), or unsuccessfully with $\uno{\pc'}$, 
...
%\end{itemize}
We also introduce iterated cell unification (\textsc{Iterated-CU}), denoted by $\rightarrow_{\cal CU}$, which extends single cell unification to lists of cells, and denote its transitive closure by $\rightarrow_{\cal CU}^*$. 
