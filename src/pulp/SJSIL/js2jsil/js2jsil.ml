...
let rec translate fid cc_table loop_list ctx vis_fid js_lab e  = 
	
	let f = translate fid cc_table loop_list ctx vis_fid None in 
    ...
    *)
	...
	let translate_to_number_binop x1 x2 jsil_aop = 
		(* x1_v := getValue (x1) with err1 *)
		let err1 = fresh_err_label () in 
		let x1_v = fresh_var () in
		let cmd_gv_x1 = SLCall (x1_v, (Literal (String getValueName)), [ x1 ], Some err1) in
		
		(* x1_n := i__toNumber (x1_v) with err2 *) 
		let err2 = fresh_err_label () in 
		let x1_n = fresh_var () in
		let cmd_tn_x1 = SLCall (x1_n, (Literal (String toNumberName)), [ Var x1_v ], Some err2) in
		...
		(* x2_n := i__toNumber (x2_v) with err4 *) 
		let err4 = fresh_err_label () in 
		let x2_n = fresh_var () in
		let cmd_tn_x2 = SLCall (x2_n, (Literal (String toNumberName)), [ Var x2_v ], Some err4) in
		...
		let new_cmds = [
			...
			(None, None, cmd_tn_x1);       (*  x1_n := i__toNumber (x1_v) with err2  *)
			(None, None, cmd_gv_x2);       (*  x2_v := getValue (x2) with err3       *)
			(None, None, cmd_tn_x2);       (*  x2_n := i__toNumber (x2_v) with err4  *)  
			(None, None, cmd_ass_xr)       (*  x_r := x1_n * x2_n                    *)
		] in 
		
		let new_errs = [
			(err1, SLBasic (SAssignment (ctx.tr_error_var, (Var x1_v))));
			(err2, SLBasic (SAssignment (ctx.tr_error_var, (Var x1_n))));
			(err3, SLBasic (SAssignment (ctx.tr_error_var, (Var x2_v))));
			(err4, SLBasic (SAssignment (ctx.tr_error_var, (Var x2_n))));
		] in 
		
		new_cmds, new_errs, x_r in 
	
	
	let translate_binop_plus x1 x2 = 
		(*  x1_v := getValue (x1) with err1  *)
		let err1 = fresh_err_label () in 
		let x1_v = fresh_var () in
		let cmd_gv_x1 = SLCall (x1_v, (Literal (String getValueName)), [ x1 ], Some err1) in
		
		(* x1_p := i__toPrimitive (x1_v) with err2 *) 
		let err2 = fresh_err_label () in 
		let x1_p = fresh_var () in
		let cmd_tp_x1 = SLCall (x1_p, (Literal (String toPrimitiveName)), [ Var x1_v ], Some err2) in
		...
		(* x2_p := i__toPrimitive (x2_v) with err4 *) 
		let err4 = fresh_err_label () in 
		let x2_p = fresh_var () in
		let cmd_tp_x2 = SLCall (x2_p, (Literal (String toPrimitiveName)), [ Var x2_v ], Some err4) in
		
		(*  goto [((typeOf x1_p) = $$string_type) or ((typeOf x2_p) = $$string_type)] then else *) 
		let then_lab = fresh_label () in 
		let else_lab = fresh_label () in 
		...
		(* then: x1_s := i__toString (x1_p) with err5 *) 
		let x1_s = fresh_var () in 
		let err5 = fresh_err_label () in 
		let cmd_ts_x1 = SLCall (x1_s, (Literal (String toStringName)), [ Var x1_p ], Some err5) in
		
		(* x2_s := i__toString (x2_p) with err6 *) 
		let x2_s = fresh_var () in 
		let err6 = fresh_err_label () in 
		let cmd_ts_x2 = SLCall (x2_s, (Literal (String toStringName)), [ Var x2_p ], Some err6) in
		
		(* x_rthen := x1_s :: x2_s  *) 
		let x_rthen = fresh_var () in 
		let cmd_ass_xrthen = SLBasic (SAssignment (x_rthen, BinOp((Var x1_s), Concat, (Var x2_s)))) in 
		
		(*   else: x1_n := i__toNumber (x1_p) with err7 *) 
		let x1_n = fresh_var () in 
		let err7 = fresh_err_label () in 
		let cmd_tn_x1 =  SLCall (x1_n, (Literal (String toNumberName)), [ Var x1_p ], Some err7) in
		
		(*  x2_n := i__toNumber (x2_p) with err8 *) 
		let x2_n = fresh_var () in 
		let err8 = fresh_err_label () in 
		let cmd_tn_x2 =  SLCall (x2_n, (Literal (String toNumberName)), [ Var x2_p ], Some err8) in
		...
		let cmds = [             
			(None, None,          cmd_gv_x1);       (*       x1_v := getValue (x1) with err1                                                        *)
			(None, None,          cmd_tp_x1);       (*       x1_p := i__toPrimitive (x1_v) with err2                                                *)
			(None, None,          cmd_gv_x2);       (*       x2_v := getValue (x2) with err3                                                        *)
			(None, None,          cmd_tp_x2);       (*       x2_p := i__toPrimitive (x2_v) with err4                                                *)  
			(None, None,          cmd_goto);        (*       goto [((typeOf x1_p) = $$string_type) or ((typeOf x2_p) = $$string_type)] then else    *)
			(None, Some then_lab, cmd_ts_x1);       (* then: x1_s := i__toString (x1_p) with err5                                                   *) 
			(None, None,          cmd_ts_x2);       (*       x2_s := i__toString (x2_p) with err6                                                   *) 
			(None, None,          cmd_ass_xrthen);  (*       x_rthen := x1_s :: x2_s                                                                *) 
			(None, None,          SLGoto end_lab);  (*       goto end                                                                               *) 
			(None, Some else_lab, cmd_tn_x1);       (* else: x1_n := i__toNumber (x1_p) with err7                                                   *) 
			(None, None,          cmd_tn_x2);       (*       x2_n := i__toNumber (x2_p) with err8                                                   *) 
		...
		let errs = [
			(err1, SLBasic (SAssignment (ctx.tr_error_var, (Var x1_v))));
			(err2, SLBasic (SAssignment (ctx.tr_error_var, (Var x1_p))));
			(err3, SLBasic (SAssignment (ctx.tr_error_var, (Var x2_v))));
			(err4, SLBasic (SAssignment (ctx.tr_error_var, (Var x2_p))));
			(err5, SLBasic (SAssignment (ctx.tr_error_var, (Var x1_s))));
			(err6, SLBasic (SAssignment (ctx.tr_error_var, (Var x2_s))));
			(err7, SLBasic (SAssignment (ctx.tr_error_var, (Var x1_n))));
			(err8, SLBasic (SAssignment (ctx.tr_error_var, (Var x2_n))))
		] in 
		
		cmds, errs, x_r in 
	
	
	let translate_binop_comparison x1 x2 is_first_first flag_arg bool_undef = 
		(* x1_v := getValue (x1) with err1 *)
		let err1 = fresh_err_label () in 
		let x1_v = fresh_var () in
		let cmd_gv_x1 = SLCall (x1_v, (Literal (String getValueName)), [ x1 ], Some err1) in
		...
		(* x_ac := i__abstractComparison (x1_v, x2_v, flag_arg) with err3  *) 
		let err3 = fresh_err_label () in 
		...
		let cmd_ac = SLCall (x_ac, (Literal (String abstractComparisonName)), args @ [ Literal (Bool flag_arg) ], Some err3) in
		...
		let cmds = [                                                          
			...
			(None, None, cmd_ac);  	                                                            (*        x_ac := i__abstractComparison (xi_v, xk_v, flag_arg) with err3; where: i != k and i,k \in {1,2} *) 
	...
	let translate_bitwise_shift x1 x2 left_fun_name right_fun_name op = 
		...
		(* x1_f := left_fun_name (x1_v) with err3 *) 
		let err3 = fresh_err_label () in 
		let x1_f = fresh_var () in
		let cmd_fc_x1 = SLCall (x1_f, (Literal (String left_fun_name)), [ Var x1_v ], Some err3) in
		
		(* x2_f := left_fun_name (x2_v) with err4 *) 
		let err4 = fresh_err_label () in 
		let x2_f = fresh_var () in
		let cmd_fc_x2 = SLCall (x2_f, (Literal (String right_fun_name)), [ Var x2_v ], Some err4) in
		...
		let cmds = [             
			(None, None,  cmd_gv_x1);       (*  x1_v := getValue (x1) with err1            *)
			(None, None,  cmd_gv_x2);       (*  x2_v := getValue (x2) with err2            *)
			(None, None,  cmd_fc_x1);       (*  x1_f := left_fun_name (x1_v) with err3     *) 
			(None, None,  cmd_fc_x2);       (*  x2_f := left_fun_name (x2_v) with err4     *)  
		  (None, None,  cmd_fc_x2);       (*  x2_f := left_fun_name (x2_v) with err4     *)
		...
		let errs = [
			(err1, SLBasic (SAssignment (ctx.tr_error_var, (Var x1_v))));
			(err2, SLBasic (SAssignment (ctx.tr_error_var, (Var x2_v))));
			(err3, SLBasic (SAssignment (ctx.tr_error_var, (Var x1_f))));
			(err4, SLBasic (SAssignment (ctx.tr_error_var, (Var x2_f))));
		] in 
	...
	| Parser_syntax.Unary_op (Parser_syntax.Not, e) ->
		...
		C(e)  =  cmds, x 
		C(!e) =  cmds
			       x_v := getValue (x) with err1;
					   x_b := toBoolean (x_v) with err2; 
						 x_r := not x_b  
     ...
     *)
		...
		let x_v = fresh_var () in 
		let err1 = fresh_err_label () in 
		let cmd_gv_x = SLCall (x_v, (Literal (String getValueName)), [ x ], Some err1) in
		
		(* x_b := toBoolean (x_v) with err2 *)
		let x_b = fresh_var () in 
		let err2 = fresh_err_label () in 
	  let cmd_tb_x = SLCall (x_b, (Literal (String toBooleanName)), [ Var x_v ], Some err2) in
		...
		let cmds = cmds @ [                        (* cmds                             *)
			(None, None, cmd_gv_x);                  (* x_v := getValue (x) with err1    *) 
			(None, None, cmd_tb_x);                  (* x_b := toBoolean (x_v) with err2 *)
			(None, None, cmd_xr_ass);                (* x_r := (not x_b)                 *) 
		] in 
		
		let errs = errs @ [
			(err1, SLBasic (SAssignment (ctx.tr_error_var, (Var x_v))));
			(err2, SLBasic (SAssignment (ctx.tr_error_var, (Var x_b)))) 
		] in 
	...
	| Parser_syntax.Unary_op (Parser_syntax.TypeOf, e) ->
		...
		C(typeof e) =           cmds
			             ...
			             next1:   goto [ base(x) = undefined] next2 next3
									 next2:   x1 := $$undefined 
								            goto next4 
									 next3:   x2 := getValue (x) with err1
									 next4:   x3 := PHI (x, x1, x2) 
									          x_r := i__js_type_of (x3) with err2
     ...
     *)
		...
		let next3 = fresh_label () in 
		let next4 = fresh_label () in 
		...
		(*  next1:   goto [ base(x) = undefined] next2 next3 *) 
		let cmd_goto_unres_ref = SLGuardedGoto (BinOp (Base x, Equal, Literal Undefined), next2, next3) in
		
		(* next3:   x2 := getValue (x) with err1 *)
		let err1 = fresh_err_label () in 
		let x1 = fresh_var () in 
		let x2 = fresh_var () in 
		let cmd_gv_x = SLCall (x2, (Literal (String getValueName)), [ x ], Some err1) in
		
		(* x_r := i__js_type_of (x3) with err2 *) 
		let err2 = fresh_err_label () in 
		let x3 = fresh_var () in 
		let x_r = fresh_var () in 
		let cmd_ass_xr = SLCall (x_r, (Literal (String jsTypeOfName)), [ Var x3 ], Some err2) in
		...
		let cmds = cmds @ [                                                                         (*             cmds                                                  *)
			...
			(None, Some next3, cmd_gv_x);                                                             (* next3:      x2 := getValue (x) with err1                          *)
			(None, Some next4, SLBasic (SPhiAssignment (x3, [| Some x_name; Some x1; Some x2 |])));   (* next4:      x3 := PHI (x, x1, x2)                                 *)
			(None, None,       cmd_ass_xr)                                                            (*             x_r := i__js_type_of (x3) with err2                   *)
		] in 
	  
		let errs = errs @ [
			(err1, SLBasic (SAssignment (ctx.tr_error_var, (Var x2)))); 
			(err2, SLBasic (SAssignment (ctx.tr_error_var, (Var x_r))))
		] in 
     ...
     *)
		...
		let err2 = fresh_err_label () in 
		let x_n = fresh_var () in
		let cmd_tn_x = SLCall (x_n, (Literal (String toNumberName)), [ Var x_v ], Some err2) in
		
		let cmds = cmds @ [                    (*            cmds                                *)
			(None, None, cmd_gv_x);              (*            x_v := getValue (x) with err1       *)
			(None, None, cmd_tn_x);              (*            x_n := i__toNumber (x_v) with err2  *)
		] in 
		
		let errs = errs @ [
			(err1, SLBasic (SAssignment (ctx.tr_error_var, (Var x_v)))); 
			(err2, SLBasic (SAssignment (ctx.tr_error_var, (Var x_n))))
		] in  	
	
		cmds, Var x_n, errs, rets
     ...
     *)
		...
		let err2 = fresh_err_label () in 
		let x_n = fresh_var () in
		let cmd_tn_x = SLCall (x_v, (Literal (String toNumberName)), [ Var x_v ], Some err2) in
     ...
     *)
		...
		(* x_v := getValue (x) with err1 *)
		let err = fresh_err_label () in 
		let x_v = fresh_var () in
		let cmd_gv_x = SLCall (x_v, (Literal (String getValueName)), [ x ], Some err) in
		...
		let errs = errs @ [
			(err, SLBasic (SAssignment (ctx.tr_error_var, (Var x_v))))
		] in 
	...
	| Parser_syntax.BinOp (e1, (Parser_syntax.Arith aop), e2) 
		...
		*) 
		...
		let new_cmds, new_errs, x_r = translate_to_number_binop x1 x2 jsil_aop in 
	...
	| Parser_syntax.BinOp (e1, (Parser_syntax.Arith Parser_syntax.Plus), e2) ->
		...
		*)  
		...
		let cmds1, x1, errs1, rets1 = f e1 in
		let cmds2, x2, errs2, rets2 = f e2 in
		let new_cmds, new_errs, x_r = translate_binop_plus x1 x2 in 
		(cmds1 @ cmds2 @ new_cmds), Var x_r, (errs1 @ errs2 @ new_errs), (rets1 @ rets2)
        ...
        C(e1) = cmds1, x1; C(e2) = cmds2, x2 
			  C(e1 << e2) =  cmds1 
				               cmds2 
											 ...
											 x2_ui32 := i__toUInt32 (x2_v) with err4
											 x_r := x1_i32 << x2_ui32
     *)
		let cmds1, x1, errs1, rets1 = f e1 in
		let cmds2, x2, errs2, rets2 = f e2 in
		let new_cmds, new_errs, x_r = translate_bitwise_shift x1 x2 toInt32Name toUInt32Name LeftShift in  
		(cmds1 @ cmds2 @ new_cmds), Var x_r, (errs1 @ errs2 @ new_errs), (rets1 @ rets2)
     ...
     *)
		let cmds1, x1, errs1, rets1 = f e1 in
		let cmds2, x2, errs2, rets2 = f e2 in
		let new_cmds, new_errs, x_r = translate_bitwise_shift x1 x2 toInt32Name toUInt32Name SignedRightShift in  
		(cmds1 @ cmds2 @ new_cmds), Var x_r, (errs1 @ errs2 @ new_errs), (rets1 @ rets2)
        ...
        C(e1) = cmds1, x1; C(e2) = cmds2, x2 
			  C(e1 >>> e2) = cmds1 
				               cmds2 
											 x1_v := getValue (x1) with err1
											 x2_v := getValue (x2) with err2
											 x1_ui32 := i__toUInt32 (x1_v) with err3
											 x2_ui32 := i__toUInt32 (x2_v) with err4
											 x_r := x1_ui32 >>> x2_ui32
     *)
		let cmds1, x1, errs1, rets1 = f e1 in
        ...
        C(e1) = cmds1, x1; C(e2) = cmds2, x2 
			  C(e1 >>> e2) =         cmds1 
				                       cmds2 
											         x1_v := getValue (x1) with err1
											         x2_v := getValue (x2) with err2
											         x_ac := i__abstractComparison (x1_v, x2_v, true) with err3 
											         goto [ x_ac = undefined ] then end 
											  then:  x_undef := false 
     ...
     *)
		let cmds1, x1, errs1, rets1 = f e1 in
		let cmds2, x2, errs2, rets2 = f e2 in
		let new_cmds, new_errs, x_r = translate_binop_comparison x1 x2 true true false in 
		(cmds1 @ cmds2 @ new_cmds), Var x_r, errs1 @ errs2 @ new_errs, (rets1 @ rets2)		
        ...
        C(e1) = cmds1, x1; C(e2) = cmds2, x2 
			  C(e1 >>> e2) =         cmds1 
				                       cmds2 
											         x1_v := getValue (x1) with err1
											         x2_v := getValue (x2) with err2
											         x_ac := i__abstractComparison (x2_v, x1_v, false) with err3 
											         goto [ x_ac = undefined ] then end 
											  then:  x_undef := false 
											  end:   x_r := PHI(x_ac, x_undef)  
     *)
		let cmds1, x1, errs1, rets1 = f e1 in
		let cmds2, x2, errs2, rets2 = f e2 in
		let new_cmds, new_errs, x_r = translate_binop_comparison x1 x2 false false false in 
		(cmds1 @ cmds2 @ new_cmds), Var x_r, errs1 @ errs2 @ new_errs, (rets1 @ rets2)	
     ...
     *)
		let cmds1, x1, errs1, rets1 = f e1 in
		let cmds2, x2, errs2, rets2 = f e2 in
		let new_cmds, new_errs, x_r1 = translate_binop_comparison x1 x2 false false true in 	
     ...
     *)
		let cmds1, x1, errs1, rets1 = f e1 in
		let cmds2, x2, errs2, rets2 = f e2 in
		let new_cmds, new_errs, x_r1 = translate_binop_comparison x1 x2 true true true in 	
		let x_r2 = fresh_var () in 
		let new_cmd = (None, None, SLBasic (SAssignment (x_r2, UnaryOp (Not, (Var x_r1))))) in 
		(cmds1 @ cmds2 @ new_cmds @ [ new_cmd ]), Var x_r2, errs1 @ errs2 @ new_errs, (rets1 @ rets2)	
     ...
     *)
		...
		let cmd_gv_e2 = SLCall (x2_v, Literal (String getValueName), [ x2 ], Some err1) in 
		(* x_ir := is_reserved (x1) *)
		let cmd_ir_e1 = SLCall (x_ir, Literal (String isReservedName), [x1], None) in 
		...
		let cmd_guarded_goto = SLGuardedGoto (is_invalid_ass_exp, err2, next) in 
		(* next: x_pv = putValue (x1, x2_v) with err3 *)
		let cmd_put_value = SLCall (x_pv, Literal (String putValueName), [x1; (Var x2_v)], Some err3) in 
		
		let cmds = 
			cmds1 @                                   (*       cmds1                                                                                      *)
			cmds2 @	[                                 (*       cmds2                                                                                      *) 
			(None, None, cmd_gv_e2);                  (*       x2_v := getValue (x2) with err1                                                            *)
			(None, None, cmd_ir_e1);                  (*       x_ir := is_reserved (x1)                                                                   *)
			(None, None, cmd_guarded_goto);           (*       goto [(((TypeOf(x1) = $$VarReferenceType) && x_ir) || (base(x1) = $$undefined))] err2 next *)
			(None, Some next, cmd_put_value)          (* next: x_pv = putValue (x1, x2_v) with err3                                                       *)  
		] in 
		
		let new_err_cmds = [
			(err1, SLBasic (SAssignment (ctx.tr_error_var, (Var x2_v)))); 
			(err2, SLCall (ctx.tr_error_var, Literal (String syntaxErrorName), [ ], None)); 
			(err3, SLBasic (SAssignment (ctx.tr_error_var, (Var x_pv))))
		] in 
		let err_cmds = List.concat [ errs1; errs2; new_err_cmds ] in 
		
		cmds, (Var x2_v), err_cmds, rets1 @ rets2
	
	| Parser_syntax.CAccess (e1, e2) -> 
		(**
      x1_v := getValue (x1) with err1;
			x2_v := getValue (x2) with err2;
			x_oc := "checkObjectCoercible" (x1) with err3;
			x2_s := "i__toString" (x2_v) with err4;
			x_r  := ref-o(x1_v, x4_v)
		...
		let cmds = 
			cmds1 @                                                                                            (*  cmds1                                           *)
			cmds2 @ [                                                                                          (*  cmds2                                           *)
			(None, None, SLCall (x1_v, Literal (String getValueName),             [ x1 ],       Some err1));   (*  x1_v := getValue (x1) with err1;                *) 
			(None, None, SLCall (x2_v, Literal (String getValueName),             [ x2 ],       Some err2));   (*  x2_v := getValue (x2) with err2;                *)
			(None, None, SLCall (x_oc, Literal (String checkObjectCoercibleName), [ x1 ],       Some err3));   (*  x_oc := "checkObjectCoercible" (x1) with err3;  *)
			(None, None, SLCall (x2_s, Literal (String toStringName),             [ Var x2_v ], Some err4));   (*  x2_s := "i__toString" (x2_v) with err4;         *)
			(None, None, SLBasic (SAssignment (x_r, (ORef ((Var x1_v), (Var x2_s))))))                         (*  x_r  := ref-o(x1_v, x2_s)                       *)
		] in  
		
		let new_err_cmds = [
			(err1, SLBasic (SAssignment (ctx.tr_error_var, (Var x1_v)))); 
			(err2, SLBasic (SAssignment (ctx.tr_error_var, (Var x2_v))));
			(err3, SLBasic (SAssignment (ctx.tr_error_var, (Var x_oc))));
			(err4, SLBasic (SAssignment (ctx.tr_error_var, (Var x2_s))))
		] in 
		let err_cmds = List.concat [ errs1; errs2; new_err_cmds ] in 
    ...
    *)
		...
		let cmd_goto = SLGuardedGoto (goto_guard_exp, err1, next) in 
		
		(* next: x_obj := toObject(base(x_e)) err2 *)
		let cmd_to_obj = SLCall (x_obj, Literal (String toObjectName), [ (Base x_e) ], Some err2) in
		
		(* x_dp := deleteProperty(x_obj, field(x_e), $$t) with err3 *) 
		let cmd_delete = SLCall (x_dp, Literal (String deletePropertyName), 
			[ (Var x_obj); (Field x_e); (Literal (Bool true)) ], Some err3) in
		
		let cmds = 
			cmds_e @ [                                (*       cmds_e                                                                 *) 
			(None, None, cmd_goto);                   (*       goto [ ((base(x_e) = $$null) or (base(x_e) = $$undefined)) ] err1 next *) 
			(None, Some next, cmd_to_obj);            (* next: x_obj := toObject(base(x_e)) err2                                      *)
			(None, None, cmd_delete)                  (*       x_dp := deleteProperty(x_obj, field(x_e), $$t) with err3               *) 
		] in 
 ...
 err_call: x_err := x_r		
     ...
     *  C(e1) = cmds1, x1; C(e2) = cmds2, x2
		...
		let cmds2, x2, errs2, rets2 = translate fid cc_table new_loop_list ctx vis_fid None e2 in
     ...
     *  C(e1) = cmds1, x1; C(e2) = cmds2, x2
		...
		let cmds1, x1, errs1, rets1 = translate fid cc_table new_loop_list ctx vis_fid None e1 in
		let cmds2, x2, errs2, rets2 = f e2 in
     ...
     *  C(e1) = cmds1, _; C(e2) = cmds2, x2; C(e3) = cmds3, _; C(e4) = cmds4, x4
		...
		let cmds4, x4, errs4, rets4 = translate fid cc_table new_loop_list ctx vis_fid None e4 in 
	...
	| Parser_syntax.Label (js_lab, e) -> 
		translate fid cc_table loop_list ctx vis_fid (Some js_lab) e 
...
let generate_main e main cc_table =
	...
	let cmds_e, x_e, errs, _ = translate main cc_table [] ctx [ main ] None e in 
	(* x_ret := x_e *)
...
let generate_proc e fid params cc_table vis_fid =
  ...
  let fid_fnames = List.map (fun f ->
	...
	let cmds_e, x_e, errs, rets = translate fid cc_table [] ctx vis_fid None e in 
