...
let print_phi_functions_per_node phi_functions_per_node = 
	...
	let str = "Printing Phi-functions per node: \n" in 
...
let insert_phi_args succ pred idom_table idom_graph phi_functions_per_node args vars cmds = 
	...
	let which_pred_str = print_which_pred which_pred in 
	...
	let new_asses_table = Array.make number_of_nodes [] in 
	...
	let rec ipa_rec u = 
		...
		let rec loop phi_nodes processed_phi_nodes = 
			...
			| (_, v_args, old_var) :: rest_phi_nodes -> 
 			...
 			| _ ->
				Printf.printf "Illegal phi-node format.\n";  
				raise (Failure "Illegal phi-node format") in 
...
let insert_phi_nodes phi_functions_per_node nodes = 
	...
	let rec create_phi_assignments u phi_nodes_u n = 
		...
		| (var, v_args, old_var) :: rest_phi_nodes -> 
				...
				create_phi_assignments u rest_phi_nodes (n + 1)  														 
  	| _ ->
				Printf.printf "Illegal phi-node format.\n";  
				raise (Failure "Illegal phi-node format") in
