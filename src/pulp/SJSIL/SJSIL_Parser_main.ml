...
let run_jsil_prog prog which_pred cc_tbl vis_tbl = 
	...
	let final_heap_str = JSIL_SExpr_Print.sexpr_of_heap heap in 
	...
	begin
		...
		if (!do_sexpr) then
			begin
				...
				let sint_prog = JSIL_SExpr_Print.sexpr_of_program int_prog false in
				...
				if (!empty_heap) then
					begin
						let str_ih_heap = JSIL_SExpr_Print.sexpr_of_heap ih_heap in 
				...
				let sprog = JSIL_SExpr_Print.sexpr_of_program prog false in
