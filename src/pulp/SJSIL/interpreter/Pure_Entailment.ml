...
let rec old_check_entailment existentials left_as right_as gamma =
	...
	let check_entailment_aux () =
		...
		begin *)
			(* Printf.printf "\nLEFT SIDE CHECK: \n%s\n" (string_of_solver solver);
			Printf.printf "\nLeft side satisfiable? %b\n" (ret = Solver.SATISFIABLE);
			Printf.printf "\nLEFT SIDE CHECK VIA CS: %b\n"
			 	(check_satisfiability old_left_as gamma existentials); *)
			Solver.push solver;
			...
			let ret = (Solver.check solver [ ]) != Solver.SATISFIABLE in
			(* Printf.printf ("Shit is SATISFIABLE: %b\n") (not ret);
			if (not ret) then
			begin
				(print_model solver);
			end; *)
			Solver.pop solver 1;
			ret, Some (solver, tr_ctx)
		(* end
		else
			(false, None) *) in
