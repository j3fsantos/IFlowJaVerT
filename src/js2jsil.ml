...
let arguments () =
  ...
  Arg.parse
    [
			...
			"-cosette", Arg.Unit(fun () -> JS2JSIL_Compiler.include_heap := false), "include setup initial heap";
    ]
...
let process_file path =
  try
	...
	let e_str = if (!test262) then JSIL_PreParser.stringify_assume_and_assert e_str else e_str in
