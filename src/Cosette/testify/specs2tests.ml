	...
	if ((List.length counter) <> (List.length preds_list)) then (
		...
		let rec loop 
			...
			| (((counter, p_name, args) :: rest_preds), symb_state) :: rest_todo_lst -> 
				try (
					Printf.printf "In the unfolding loop with the predicate %s with counter %d\n"
						p_name counter; 
					...
					let new_symb_states : symbolic_state option list = 
						List.map 
							(fun (_, pred_symb_state, _) -> 
								(Spatial_Entailment.unfold_predicate_definition caller_store subst_e 
									pat_subst SS.empty spec_vars pred_symb_state symb_state) 
					...
					let new_todo_list = List.map (fun ss -> rest_preds, ss) new_symb_states in 
