...
;  prog heap store ctx cur-index next-prev-index
         ...
         (let ((outcome (car (run-proc prog call-proc-name heap arg-vals))))
           ...
           (cond
             ...
             [(eq? (first outcome) 'normal)
              ...
              (let ((new-store (mutate-store store lhs-var (second outcome))))
                (run-cmds-iter-next prog proc-name heap new-store cur-index cur-index))]
...
(define (run-proc prog proc-name heap arg-vals)
  (if (has-racket-implementation? proc-name)
      ...
      (let* ((proc (get-proc prog proc-name))
        ...
        (let ((heap (mutate-heap heap larguments parguments (make-jsil-list arg-vals))))
