...
let create_info_node_aux symb_state new_node_number cmd_index cmd_str =
	let heap_str = Symbolic_State_Print.string_of_shallow_symb_heap (ss_heap symb_state) true in
	let store_str = Symbolic_State_Print.string_of_shallow_symb_store (ss_store symb_state) true in
	let pfs_str = Symbolic_State_Print.string_of_shallow_p_formulae (ss_pfs symb_state) true in
	let gamma_str = Symbolic_State_Print.string_of_gamma (ss_gamma symb_state) in
	let preds_str = Symbolic_State_Print.string_of_preds (ss_preds symb_state) true in
	let new_node_info =
		{
			heap_str = heap_str;
			store_str = store_str;
