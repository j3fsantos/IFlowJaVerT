...
let rec auto_unfold
	...
	| LMetaData _ | LTypes _ | LEmptyFields _ | LSetMem (_, _) | LSetSub (_, _) | LForAll (_, _) -> [asrt]
...
let detect_trivia_and_nonsense (u_pred : unfolded_predicate) : unfolded_predicate =
	let new_definitions = List.map
		(fun (oc, x) -> oc, (reduce_assertion_no_store_no_gamma_no_pfs x)) u_pred.definitions in
