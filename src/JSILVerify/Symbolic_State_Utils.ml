...
let convert_symb_state_to_assertion
  ...
  assertions in
   ...
   | true -> assertion_substitution assertion subst true
   | false -> assertion
...
let string_of_single_spec_table_assertion single_spec =
	let pre = convert_symb_state_to_assertion single_spec.n_pre true in
	let post = convert_symb_state_to_assertion (List.hd single_spec.n_post) true in
