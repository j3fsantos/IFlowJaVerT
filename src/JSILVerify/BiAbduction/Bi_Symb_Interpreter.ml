...
and
symb_evaluate_logic_cmds s_prog (l_cmds : jsil_logic_command list) (symb_states_with_spec_vars : (symbolic_state * SS.t) list) subst ?lemm_depd_graph : (symbolic_state * SS.t) list =
		...
		let new_symb_states_with_spec_vars =
			List.fold_left
				(fun ac_new_symb_states_with_spec_vars (symb_state, spec_vars) ->
					let new_symb_states_with_spec_vars = symb_evaluate_logic_cmd s_prog l_cmd symb_state subst spec_vars lemm_depd_graph in
			...
			if (is_visited next) then
				...
				let symb_state =
					...
					| Some a ->
						...
						(match (Structural_Entailment.grab_resources symb_state new_symb_state spec.n_lvars SS.empty) with
