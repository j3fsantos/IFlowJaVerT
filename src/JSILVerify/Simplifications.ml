 ...
 *)
let simplify_symbolic_state 
	(vars_to_save : string list)
	 ...
	 *)
	let lvars = get_lvars_from_symb_state symb_state in	
	let lvars_gamma = get_lvars_from_gamma gamma in		
	let lvars_inter = SS.inter lvars lvars_gamma in
	Hashtbl.filter_map_inplace (fun v t ->
		(match (SS.mem v lvars_inter || List.mem v vars_to_save) with
		| true  -> Some t
		| false -> 
	...
	Hashtbl.iter (fun v t -> 
		let lexpr = (match t with
			...
			| _ -> None) in
		(match lexpr with
		| Some lexpr -> Hashtbl.add subst v lexpr;
		| None -> ())) gamma;
	(* And substitute, if allowed *)
	let symb_state = Symbolic_State_Basics.symb_state_substitution symb_state subst true in
	let other_pfs = Symbolic_State_Basics.pf_substitution other_pfs subst true in
	Hashtbl.iter (fun v _ ->
		match (List.mem v vars_to_save) with
		| true -> ()
		| false -> while (Hashtbl.mem gamma v) do Hashtbl.remove gamma v done
		) subst
