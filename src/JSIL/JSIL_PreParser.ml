...
let rec stringify_assume_and_assert_aux (s_in : string) (s_out : string) (s : state) : string =
	...
	| "" -> 
		...
		| InAsmOrAsrt _ -> raise (Unparseable "Reached EOF: assume or assert not terminated.")
		| InAsmOrAsrtAndInString _ -> raise (Unparseable "Reached EOF: a string literal inside assume or assert not terminated."))
	| _ -> 
			...
			| InAsmOrAsrt (depth, str) ->
					...
					| "(" -> (match depth with
						...
						| _ -> f rest_of_string s_out (InAsmOrAsrt (depth + 1, str ^ next_char)))
					| ")" -> 
							(match depth with
							| 1 -> 
							...
							| _ -> f rest_of_string s_out (InAsmOrAsrt (depth - 1, str ^ next_char)))
					| " " | "\t" when depth = 0 -> f rest_of_string (s_out ^ next_char) (InAsmOrAsrt (depth, str))
					| _ -> (match depth with
						| 0 -> raise (Unparseable ("Parsing error: assume or assert not followed by an open parenthesis but by " ^ next_char ^ "."))
						| _ -> (match next_char with
							| "\"" ->  f rest_of_string s_out (InAsmOrAsrtAndInString (depth, str ^ next_char))
							| _ -> f rest_of_string s_out (InAsmOrAsrt (depth, str ^ next_char))))
					)
			| InAsmOrAsrtAndInString (depth, str) ->
  				...
  				| "\\" -> 
  						...
  						| Some nc, Some rest_of_string -> f rest_of_string s_out (InAsmOrAsrtAndInString (depth, str ^ "\\" ^ nc)))
  				| _ -> f rest_of_string s_out (InAsmOrAsrtAndInString (depth, str ^ next_char)))
			)
	)

let stringify_assume_and_assert (s : string) = stringify_assume_and_assert_aux s "" TheUsual