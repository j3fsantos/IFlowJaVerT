...
our symbolic analysis, including: \dtag{i} a soundness result for the symbolic 
execution, \dtag{ii} a guarantee of absence of false positives for bug-finding, 
and \dtag{iii}  \polish{a verification result that precisely states the conditions 
...
of the implementation of the \jsil symbolic analysis. %in \rosette~\cite{Rosette1,Rosette2}.
...
\begin{tabular}{lllll}
	...
	Types: $\jtype \in \jtypes$ & \multicolumn{4}{l}{Values: $\val \in \vals$ \defeq \ $\jnumber \mid \jbool \mid \jstring \mid  \jundefined \mid \jnull \mid \loc \mid \jtype \mid \pid \mid \jempty$} \\[0.1cm]
\multicolumn{5}{l}{Expressions: $\jsilexpr \in \exprs$ \defeq \ $\val \mid \jvar \mid \ominus\ \jsilexpr \mid \jsilexpr \binop{} \jsilexpr $} \\[0.1cm]
	...
	% Commands
	\multicolumn{5}{l}{Commands: $\jcmd \in \cmds$ \defeq \ $ \bcmd \mid \goto \ i \mid  \ifgoto{\jsilexpr}{i}{j} \mid \jsilcall{\jvar}{\jsilexpr}{\jvec{\jsilexpr}}{j} 
	         \mid \assume(\jsilexpr) \mid \assert(\jsilexpr)$} \\[0.1cm]
	\multicolumn{5}{l}{Procedures : $\proc \in \procs$ \defeq \ $\procedure{\pid}{\jvec{\jvar}}{\jvec{\jcmd}}$}
 \end{tabular}}
\end{display}
...
\inferrule[\textsc{Assignment}]
  ...
  }{\absbsemrule{\absstate, \jvar := \jsilexpr}{\stupdt(\absstate, \jvar, \jsilexpr) }} 
\\
...
\inferrule[\textsc{Property Assignment}]
  ...
  }{\absbsemrule{\absstate, [\jsilexpr_1, \jsilexpr_2] := \jsilexpr_3}{\hpupdt(\absstate', \absloc, \absprop, \jsilexpr_3)}} 
\and
...
This approach to the design of symbolic analyses has the following benefits: \dtag{1} it avoids redundancy,
\dtag{2} it makes the choices in the design of the symbolic semantics more explicit, 
and \dtag{3} it leads to more modular implementations by avoiding code duplication.  
...
\begin{itemize}
  ...
  \item \emph{GetCell} ---  $\getcell \subseteq \absstates \times \exprs \times \exprs \times \absstates \times \abslocs \times \absvals \times \absvals$. 
          ...
          \dtag{2} $\absprop$ denotes the field name resulting from the evaluation of $\jsilexpr_2$, 
          \dtag{3} $\absval$ denotes the value of the field $\absprop$ of the object at location $\absloc$, 
          and \dtag{4} $\absstate'$ denotes the re-arrangement of $\absstate$ to allow for the field inspection. 
...
and \dtag{5} $j$ the index to which it must jump in case of error return. 
...
%Transitions for basic commands, given in Figure \ref{fig:sem:basic:commands}, are of the form $\semtrans[\mode][\mode']{\heap, \store, \bcmd}{\heap', \store'}$, meaning that the execution of the basic command $\bcmd$ in the heap $\heap$, store $\store$, and execution mode $\mode$ results in the heap $\heap'$, $\store'$, and execution mode $\mode'$. 
...
%Figure~\ref{abs:sem:bcmds:fig} presents the abstract semantics of basic commands, whereas 
%Figure~\ref{abs:sem:cmds:fig} presents the abstract semantics of control flow commands. 
%
