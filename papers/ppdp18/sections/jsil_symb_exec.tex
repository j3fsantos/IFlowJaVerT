...
%
The key observation is that by having the instrumented semantics as a proper interim stage in the design of the analysis, we obtain 
...
%
We design the instrumented semantics in the style of~\cite{gardner:popl:2012,javert}, by explicitly keeping track of properties that are not present in a given object.
...
\end{minipage}
 \vspace*{-0.4cm}
\caption{Abstract semantics of \jsil basic commands (left) and \jsil commands (right)}\label{abs:sem:cmds:fig}
\vspace*{-0.3cm}
\end{figure*}

\subsection{Syntax and Abstract Semantics}\label{subsec:jsil:analysis:formalism}
...
A \jsil program $\prog \in \progs$ is a set of top-level procedures of the form $\procedure{\pid}{\jvec{\jvar}}{\jvec{\jcmd}}$, where $\pid$ is the procedure identifier, $\jvec{\jvar}$ are its formal parameters, and its body ${\jvec{\jcmd}}$  is a sequence of \jsil commands.
...
%             
  ...
  \item[GetDomain,] $\getdomain : \absstates \rightarrow \exprs \partialmap \absvals$. 
          % We pretty-print $(\absstate, \jsilexpr, \absval) \in \getdomain$ as $\absgetdomainrule{}{\absstate, \jsilexpr}{\absval}$. 
           ...
           in $\absstate$. GetDomain will be essential for the treatment of 
...
in Figure~\ref{abs:sem:cmds:fig}. Transitions for 
...
\jfs{
\begin{itemize}
  \item Explain the {\color{red}RED}
\end{itemize}
}
...
%  \and
 % 
   ...
   }{  \absgetcellrule{\symbolic}{(\sheap, \sdom, \sstore, \pc), (\jsilexpr_1, \jsilexpr_2)}{(\sheap', \sdom', \sstore,  {\color{red}\pc \, \wedge \, \sexprp \not\in (\sdom(\sloc) \cup \jsilset{\sexprp_i \mid_{i=0}^m}})), (\sloc, \sexprp, \none)}}
  %
  \\
  %
    \inferrule[\textsc{GetCell - Found}]
   { 
      ...
      \\\\
       ...
       %\pc \not\vdash \sexprp' \neq \sexprp 
       %\quad 
       \sstate' = (\sheap, \sdom, \sstore, {\color{red} \pc \wedge (\sexprp = \sexprp')})
...
\begin{wrapfigure}{R}{0.15\textwidth}
{\small
\hspace*{-0.5cm} $\mathtt{0\quad s := symbolic\ (String)}$ \\
\hspace*{-0.5cm} $\mathtt{1\quad o := new\ ()}$ \\
\hspace*{-0.5cm} $\mathtt{2\quad o[s] := 42};$ \\
\hspace*{-0.5cm} $\mathtt{3\quad x := getFields(o);}$ \\
\hspace*{-0.5cm} $\mathtt{4\quad assert\ (card \ x == 2)}$
}
\end{wrapfigure}
...
The last step of our methodology consists of  linking the symbolic semantics to the concrete semantics
...
\inferrule[Symbolic Domains]
{
  \hdom =  \left\{ (l, p) \mid 
                     {\begin{array}{l}
                         \sexprl \in \domain(\sdom) \, \wedge \, l = \interpret{\symbconc}{\senv}(\sexprl)  \\ 
...
execution with respect to the concrete semantics. Note that this theorem quantifies over all possible 
frames, which is not the case for standard results  in symbolic execution. 

\begin{theorem}[Bounded Sounded + Frame Resilience]\label{teo:soundness:jsil:symb:exe}
$$
\begin{array}{l}
...
interpreter described in \S\ref{subsec:symb:semantics}. %consistent with the symbolic semantics described in  \S\ref{subsec:jsil:analysis:formalism}. 
