...
\polish{When calling a procedure with not all the required formal parameters, the missing parameters are implicitly set to $\jundefined$.}
...
Note that the positive and negative heap update rules are not applicable at the same time, since $\val$ cannot be $\none$. 
...
%
To achieve this, the instrumented semantics keeps track of both the present \emph{and the absent} properties of a given object, 
...
If $\idom(\loc)$ is not defined, then all properties of the object at $\loc$ that are not in the heap \emph{can be} safely framed~on. Also, any properties in $\idom(\loc)$ that are not in the heap can be safely framed on, as there is nothing is stated about them.
