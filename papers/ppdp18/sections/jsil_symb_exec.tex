...
that, given a symbolic trace, $\prog : \transabssemrule{\sstate, \scs, i}{\sstate', \scs', j}{\mode}{\mode'}{\symbolic}$,
 ...
 instrumented state because we only {\color{red}care} about the initial instrumented states for which 
...
We implement the \emph{instrumented} \jsil interpreter of \S\ref{subsec:instrumented} in \rosette, obtaining a lifted \jsil symbolic interpreter for free. We show that this symbolic interpreter is consistent with the symbolic semantics given in \S\ref{subsec:symb:semantics}, as described in \S\ref{sec:intro}.
...
To illustrate how the lifting works, we restate Corollary~\ref{bug:finding} for JavaScript.
...
If there is an assertion failure at the \jsil level, then, by the correctness of the compiler, 
