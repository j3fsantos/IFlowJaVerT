...
is parametric on a \jsil state signature. We are not doing abstract interpretation.} in the style of~\cite{vanhorn:icfp:2010}. 
This abstract semantics is the bedrock for both the formal development \emph{and} the implementation of the analysis. From it, we obtain the concrete, instrumented, and symbolic semantics by instantiation. 
...
\end{mathpar}}
\vspace*{-0.4cm}
\caption{Abstract semantics, basic commands: {\small $\absbsemrule{\absstate, \bcmd}{\absstate'}{}$}}\label{abs:sem:bcmds:fig}
\vspace*{-0.53cm}
\end{figure}
...
\caption{Abstract semantics, commands: {\small $\abssemrule{\absstate, \abscs, i}{\absstate', \abscs', j}{\mode}{\mode'}{}$}}\label{abs:sem:cmds:fig}
\vspace*{-0.7cm}
\end{figure}
...
We denote: the empty heap by~$\hemp$; heap lookup by $\heap(\loc, \jstring)$; 
...
for the required abstractions below.
...
\pmax{What happen with symbsem? This is normal, etc. Symbolic lifting. Mention frame. Mention d can be undefined}
...
\pmax{Mention symbolic branching}
...
Our three semantics for \jsil (\S\ref{subsec:concr}--\S\ref{subsec:symb:semantics}) are specifically designed with implementation in mind. To implement any of them, one only needs to follow the rules, which are written operationally, modularly, and are syntax-directed. \polish{Contrast with related work.}

However, implementing the entailment engine required by the symbolic semantics and practically handling potential combinatorial explosions is a more challenging task. For this reason, our proof-of-concept, prototype version of \cosette leverages on 
\rosette~\cite{Rosette1,Rosette2}, a symbolic virtual machine enabling the development of solver-aided languages. \rosette extends Racket~\cite{racket} with symbolic values and first-order assertions over these values, and languages implemented in \rosette can make use of its solver-aided facilities. 
