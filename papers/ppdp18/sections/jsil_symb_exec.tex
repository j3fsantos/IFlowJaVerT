...
GetCell extends its instrumented counterpart by recording relevant information in the path condition (highlighted in blue): if a symbolic property is found, we record the appropriate equality; if it is not found, we record that it is different from all of the properties. 
...
\myparagraph{Non-determinism} The symbolic semantics is highly non-determi-nistic. 
This is evident in the definition of GetCell, where the property $\sexprp$ is non-deterministically
chosen to be equal to any of the properties of the object at location $\sexprl$ (\textsc{GetCell - Found}), but also to be different from all of them (\textsc{GetCell - Not Found}).
This is in contrast to the concrete and instrumented semantics, where the only source of non-determinism is object allocation. 
