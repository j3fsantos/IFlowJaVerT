...
\jsil~\emph{expressions}, $\jsilexpr \in \exprs$, include \jsil values, \jsil program variables $\jvar$, typed symbolic variables $\svar$, and various unary and binary operators (e.g.~for supporting sets and lists).  
%
For the typed symbolic variables, we use $\sstring$, $\snumber$, $\sbool$, and $\sloc$ to denote symbolic strings, numbers, booleans, and locations, respectively.
...
Compositional analyses need to be able to reason about programs given partial state information,
which is challenging for languages that do not observe the frame property.
We approach this problem by designing an instrumented version of the language semantics that \emph{does} 
observe the frame property. To achieve this, we keep track of not only the properties that are present in a given object, but also the properties that are not present.
...
Furthermore, if $(\loc, p) \in \domain(\iheap)$, then $p$ must be in $\idom(\loc)$.
If $\idom(\loc)$ is not defined, then all properties of the object at $\loc$ that are not in the heap \emph{can be} framed~on. Also, all properties in $\idom(\loc)$ that are not in the heap can be safely framed on, as there is effectively no information about them.
...
\pmax{What happen with symbsem? This is normal, etc. Symbolic lifting. Mention frame. Mention d can be undefined}
 ...
 We further introduce symbolic expressions, $\sexpr \in \sexprs$, defined as follows: 
