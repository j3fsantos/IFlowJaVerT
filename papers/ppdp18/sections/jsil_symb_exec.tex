...
\jsil~\emph{expressions}, $\jsilexpr \in \exprs$, include \jsil values, \jsil program variables $\jvar$, typed symbolic variables $\svar$, and various unary and binary operators (e.g.~for supporting sets and~lists). Symbolic variables range over symbolic strings, numbers, booleans, and locations, denoted respectively by $\sstring$, $\snumber$, $\sbool$, and~$\sloc$.
...
\myparagraph{Abstract semantics} We design the abstract semantics to make the analysis, the proofs, and the implementations as modular as possible. At its core are the GetCell and GetDomain functions, which precisely pinpoint the way in which \jsil interacts with the heap, factoring out the common behaviour of \jsil commands. It also provides constructs for standard reasoning about symbolic values, whose concrete and instrumented semantics are trivial.
...
In the following, we use the special symbol $\none$ (read: none) to denote the absence of a property in an object, and use $\setext{\absvals}{\none}$ for the set $\absvals \cup \{ \none \}$.
...
%             
   ...
   \item[Heap Update,] $\hpupdt{}(\absstate, \absloc, \absprop, \absval)$, denoting the state obtained from~$\absstate$ by updating the value of property $\absprop$ of the object at location $\absloc$ to $\absval$ or creating that property, in the
             case it does not exist.

  \item[GetCell,] $\absgetcellrule{}{\absstate, \jsilexpr_1, \jsilexpr_2}{\absstate', (\absloc, \absprop, \absval)}$, which retrieves the value associated with a given property of a given object. Formally, if $\absgetcellrule{}{\absstate, \jsilexpr_1, \jsilexpr_2}{\absstate', (\absloc, \absprop, \absval)}$ holds, 
...
\inferrule[\textsc{Object Creation}]
  { 
     \absalloc{}(\absstate) = (\loc, \absstate') \quad \absgetcellrule{}{\absstate', \loc, \protop}{\absstate'', -} \\\\ \absstate''' = \hpupdt{}(\absstate'', \loc, \protop, \jnull)
...
\inferrule[\textsc{Member Check - True}]
  { 
     \absgetcellrule{}{\absstate, \jsilexpr_1, \jsilexpr_2}{\absstate', (-, -, \absval)}
     \\\\
    \absval \neq \none \quad \absstate'' = \stupdt{}(\absstate', \jvar, \jtrue)
...
\vspace*{-0.2cm}
{\footnotesize
\hspace*{-0.47cm} $\mathtt{0\quad w := \hasfield(x, \litstring{get})}$ \\[-0.08cm]
\hspace*{-0.47cm} $\mathtt{1\quad \ifgoto{w}{4}{2}}$ \\[-0.08cm]
\hspace*{-0.47cm} $\mathtt{2\quad x := [x, \litstring{\protop}] }$ \\ [-0.08cm]
\hspace*{-0.47cm} $\mathtt{3\quad \ifgoto{x = \jnull}{\errlab}{0}}$ \\[-0.08cm]
\hspace*{-0.47cm} $\mathtt{4\quad y := [x, \litstring{get}]}$ \\[-0.08cm]
\hspace*{-0.47cm} $\mathtt{5\quad \jsilcall{z}{y}{}{\errlab}}$ 
}
\vspace*{-0.2cm}
 ...
 % 
   ...
   { 
        ...
        \quad 
         ...
         \quad
       p \not\in \idom(\loc) 
            }{  \absgetcellrule{\instrumented}{\istate, \jsilexpr_1, \jsilexpr_2}{(\iheap \dunion (\loc, p) \mapsto \none, \idom[\loc \mapsto \idom(\loc) \cup \jsilset{p}], \store), (\loc, p, \none)}}
  %
  \\
  \inferrule[\textsc{GetDomain}]
   { 
...
only means that this property does not exist in the current heap. Also, note how GetDomain collects all non-none properties of an object by inspecting both the heap and the domain table. %We note that the instrumented semantics does not evaluate symbolic variables.
...
\vspace*{+0.05cm}
{\small
\hspace*{-0.5cm} $\mathtt{0\quad o := \jsilnew ()}$ \\[-0.06cm]
\hspace*{-0.5cm} $\mathtt{1\quad [o, "foo"] := 42}$ \\[-0.06cm]
\hspace*{-0.5cm} $\mathtt{2\quad \jsildelete(o, "foo")}$ \\ [-0.06cm]
...
\noindent Symbolic variables evaluate to themselves: $\evalexpr{s}(\svar) = \svar$. As they carry meaning only in the symbolic semantics, their evaluation is not supported by the concrete and instrumented semantics (cf. Appendix). 
GetCell extends its instrumented counterpart by recording relevant information in the path condition (highlighted in blue): if a symbolic property is found, we record the appropriate equality; if it is not found, we record that it is different from all of the properties. We note that the evaluation of a symbolic variable $\svar$ returns $\svar$ itself.
...
\hspace*{-0.55cm} $\mathtt{3\quad x := getProps(o);}$ \\
\hspace*{-0.55cm} $\mathtt{4\quad assert\ (card \ x = 2)}$
}
\vspace*{-0.2cm}
\end{wrapfigure}

\pmax{Non-determinism! Abstract not deterministic - concrete/instruments - object allocation, here real non-determinism.}
...
a (concrete) heap frame that can be safely added to $\jstate$, meaning that it does not overlap with the negative resource captured by~$\sstate$. 

\pmax{Explain I for heaps and domains. Explain how it fits the frame.}
...
\transabssemrule{\sstate, \scs, i}{\sstate', \scs', j}{\mode}{\mode'}{\symbolic} 
    ...
    \ \implies \ \exists \, \senv' \geq \senv, \jstate', \cs' \, . \, 
...
We implement the \emph{instrumented} \jsil interpreter of \S\ref{subsec:instrumented} in \rosette, obtaining a \jsil symbolic interpreter for free. To make certain that this symbolic interpreter is consistent with the symbolic semantics of \S\ref{subsec:symb:semantics}, we systematically construct symbolic unit tests for each \jsil command, assuming the premises and asserting the conclusion of the appropriate rule of the symbolic semantics. For lack of space, more information on these tests is available in the Appendix.
