...
\dtag{1}~$\pid$~is the identifier of the executing procedure;
\dtag{2}~$\absstore$~is the store of the procedure that called~$\pid$; 
\dtag{3}~$\jvar$~is the variable that will hold the return value of $\pid$ in~$\absstore$; 
\dtag{4} $i$ is the command index to which the control jumps after $\pid$ terminates normally; 
and \dtag{5} $j$ is the command index to which it jumps in case of an error. 
...
only means that this property does not exist in the current heap. Also, note how GetDomain collects all non-none properties of an object by inspecting both the heap and the domain table.
 ...
 that the frame bug illustrated in the middle column cannot be replicated in the instrumented settin.,  
 Also, if we were to remove $(\loc_o, \litstring{get}) \mapsto \none$ from the initial heap, 
...
for the required abstract functions, focussing below on the \polish{Sth}. % (we omit those that coincide with the instrumented case). 
