...
\Function{Unification}{$\sstate$, $\up$, $\subst$}
    ...
    \State $\qquad \qquad \textbf{match} \ \unifylexpr(\sexprv, \sexprv', \subst, \sstate.\pcsel)$ $\textbf{with}$
     ...
     \State $\mid~\sass \lstcons \up' :$   
      \State $\qquad \textbf{if} \, (\fv(\sass) \not\subseteq \domain(\subst))$ %\textbf{ and } (\sstate.\pcsel \vdash \subst(\sass)))$ 
      \State $\qquad \qquad \textbf{then}$ \Call{Unification}{$\sstate'$, $\up'$, $\subst$}
\EndFunction
\end{algorithmic}
\end{algorithm}
...
%\State $lla  := InferSpec$
%\ForAll{$ Bla$}
%\If{$Q_f \sep Q_M \vdash Q \sep \jtrue$}
%  \State $BLA $
%\EndIf
%\EndFor
%\EndFor
