...
\jfs{
\begin{itemize}
   \item Symbolic states are assertions 
   \item Assert-* 
\end{itemize}
}
    ...
    \State $\qquad \qquad \textbf{match} \ \unifylexpr(\sexprv, \sexprv', \subst, \sstate.\pcsel)$ $\textbf{with}$
     ...
     \State $\mid~\sass \lstcons \up' :$   
      ...
      \State $\qquad \qquad \textbf{else}$ $\textbf{if} \,(\sstate.\pcsel \vdash \subst(\sass))$
       \State $\qquad \qquad \qquad \textbf{then}$ \Return  \Call{Unification}{$\sstate$, $\up'$, $\subst$}
...
\vspace{2pt}
\begin{display}{Verification Rules}
\text{
{\scriptsize
\begin{mathpar} 
  ...
  \\\\
   ...
   }{  \absgetcellrule{\symbolic}{ , \sloc, \sexprp}{\sstate', \sexprv}}
\quad
   ...
   }{  \absgetcellrule{\symbolic}{(\sheap, \sdom, \sstore, \pc), (\jsilexpr_1, \jsilexpr_2)}{(\sheap', \sdom', \sstore,  {\color{blue}\pc \, \wedge \,)), (\sloc, \sexprp, \none)}}}
 \end{mathpar}}}
 \end{display}
