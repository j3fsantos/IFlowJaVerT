...
We conclude the section with a discussion on how to lift the presented results to JavaScript (\S\ref{subsec:liftmejs2}).
...
\dtag{1} maps all program variables in $\pass$ to freshly generated logical variables of the appropriate type obtaining a symbolic store $\sstore$;
\dtag{2} replaces all program variables in $\pass$ with their bindings given by $\sstore$, obtaining an assertion $\pass'$ with no program variables; and
\dtag{3} collects all the cell assertions in $\pass'$ to form the symbolic heap,
