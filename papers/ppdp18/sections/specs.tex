...
\begin{itemize}
   ...
   \item $\jvec{\spass} = (\sloc, \sexprp) \pointsto \sexprv \lstcons \jvec{\sqass}$ ---
            the algorithm uses $\GetCellVFun$ (described below) to obtain the symbolic value 
            $\sexprv'$ associated with $(\sloc, \sexprp)$ in the current symbolic state and
            checks that $\sexprv' = \sexprv$ under the current path condition $\sstate.\pcsel$. 
            If the entailment holds, the unification continues. If not, the algorithm generates 
            the \emph{failing constraint} $\sexprv' \neq \sexprv$. 
  
   \item $\emptyfields{\sloc}{\sexprv} \lstcons \jvec{\sqass}$ ---
            the algorithm uses $\GetDomainVFun$ (described below) to obtain the domain $\sexprv'$
            of $\sloc$ in the current symbolic state and 
            checks that $\sexprv' \subseteq \sexprv$ under the current path condition $\sstate.\pcsel$. 
            \polish{
              For instance, 
            ...
            resource not captured by $\emptyfields{\sloc}{\sexprv}$ and the unification continues. 
   ...
   \item $\jvec{\spass} = \sass \lstcons \jvec{\sqass}$ --- 
            the algorithm checks that $\sass$ is entailed by the current path condition. 
             If it is, the unification continues. If not, the algorithm generates 
