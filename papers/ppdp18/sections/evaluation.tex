...
Throughout this section, we will be using the Buckets.js~\ref{buckets} (which has around 65k downloads on npm) data structure library as a running example.
...
With the help of \cosette, we were able to find a bug in the implementation of the Buckets.js \jsinline{multidictionary} (a key-value map in which a single key might hold several values), as well as a bug in \jsinline{queue-pri}, a JS priority queue implementation~(\ref{queue-pri}). 
