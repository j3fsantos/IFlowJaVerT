...
\affiliation{$^1\ $Imperial College London, $^2\ $ENS Paris, $^3\ $IBM New York}
...
JavaScript is the most widespread dynamic language, used by 94.8\% of websites~\cite{JS948percent}.
Due to its dynamic, complex nature, it is known to be a difficult target for symbolic analysis. 
However, recent symbolic execution tools for JavaScript~\cite{saxena:sp:2010,wittern:icse:2018,li:fse:2014} have made significant progress. 
...
They are fully automatic, aim at code in the large, and primarily focus on scalability and coverage issues. On the other hand, they target only specific bug patterns and are underpinned by whole-program analysis. 

Whole-program analysis is not a good fit for JavaScript applications, 
which are commonly run in highly dynamic execution
environments: for example, client-side programs dynamically load and
execute third-party code as a matter of course. Instead, one
needs to be able to analyse incomplete code in a compositional manner.
...
% what we've done.}
  ...
  is fully formalised in a way that facilitates implementation and is
  also {\em trusted}, in that is provably follows the JavaScript
...
\myparagraph{Application: Whole-Program Symbolic Testing} 
...
and also discover bugs in each of the libraries.
...
Full functio-nal correctness 
specifications of JavaScript programs are highly intricate, with only a few tools (e.g.,~\javert~\cite{javert} and KJS~\cite{Park:2015,stefanescu:oopsla:2016}) supporting such expressivity. 
...
understand in detail a complicated proof trace. 
...
separation logic specifications of JavaScript programs in \javert, due
...
Because it is trustworthy, \cosette can be used both as a basis for building other more specific analysis 
(e.g. combining symbolic analysis with a type-based analysis to increase the precision of the latter) and 
as a testing oracle for other symbolic execution tools for JavaScript that purposely 
...
\polish{frame resilience guarantees}, which are essential for generating tests from specifications. 
%None of them can be used to test JS specs. 
...
\emph{Quickcheck}~\cite{claessen:icfp:2000}. \emph{Quickcheck} and its followers~\polish{\cite{CITE}}
...
JML specifications~\polish{\cite{CITE}}, but it requires the class code to include 
...
tests for Java code annotated with relational logic specifications. 
 ...
 %
 However, the resulting tests would be less effective from the point of view of bug finding; therefore, we opted not to do this. 
