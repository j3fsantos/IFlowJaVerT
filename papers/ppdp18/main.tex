...
\pgmaxinline{What's a framework, what's a toolchain, what's a tool}

\pmax{Something is a framework when we don't want to keep using toolchain across papers.
Something else is a toolchain because we need the T in the acronym to make sense.}
...
websites\footnote{See {\tiny  \texttt{w3techs.com/technologies/details/cp-javascript/all/all}}.}.
...
extended. In JavaScript,  it is possible to introduce bugs
...
We introduce \cosette, a compositional symbolic execution toolchain 
for JavaScript  (ECMAScript 5
Strict~\cite{ecma} \footnote{It is straightforward to extend this
  work to the non-strict case. Later extensions and revisions contain
  ECMAScript 5 as a common core.}).  The architecture of \cosette is
shown on the right. It comprises  an extension of the
\JSComp
compiler~\cite{.}  from JavaScript to the intermediate language JSIL,
to account for symbolic values and assertions, and a symbolic
interpreter for JSIL,  implemented using
Rosette~\cite{Rosette2,Rosette1}.  The JSIL symbolic interpreter either outputs a
concrete counter-model for the given assertions, or guarantees
correctness up to a given bound for unfolding loops and
...
does not satisfy the frame property. It can be described in three
steps: \dtag{1} design an JSIL instrumented  semantics that exhibits the
frame property by explicitly keeping track of properties that must
{\em not be present} in a given object in order for the program to
behave correctly; \dtag{2} define the JSIL symbolic semantics by
lifting the JSIL instrumented semantics; and \dtag{3} link the 
JSIL symbolic semantics to the  concrete JSIL semantics by describing the
frames that can be safely added to the initial state.
%
...
An essential goal for us is to establish trust in Cosette
toolchain. The symbolic interpreter for JSIL comprises a concrete
implementation of the instrumented JSIL semantics, which is
automatically lifted to an implementation of the JSIL symbolic
semantics using Rosette. The formal JSIL instrumented semantics and
its lifting are carefully designed to mirror the implementation. To
provide further corroboration of the JSIL symbolic interpreter, we
systematically constructed and successfully ran symbolic tests for
each JSIL command, assuming the premises and asserting the conclusion
of the appropriate rule of the symbolic semantics.  To ensure that
