...
framework for JavaScript, combining the JS-2-JSIL compiler~\cite{.}
and our JSIL symbolic interpreter written in Rosette~\cite{.}. We
...
We have developed the methodology for designing compositional program
...
semantics for JSIL, which we instantiate to obtain concrete,
instrumented, and symbolic semantics. This abstract semantics is the
bedrock for both the theoretical results and the implementation of the
analysis. We prove that the symbolic execution underpinning Cosette is
sound, frame resiliant and does not generate false positives. We
establish additional trust by using the theory to precisely guide the
implementation and by thorough testing.

Our JSIL symbolic interpreter was written in Rosette, a good first
choice in the development of Cosette. We have applied Cosette to real-world code, but have now hit a
bottleneck due to the performance of Rosette. This is because the
Rosette code is interpreted rather than run natively. In future, we
will implement our own symbolic execution tool from scratch, using the
abstract semantics for JSIL presented here.
