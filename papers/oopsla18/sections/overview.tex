...
that checks that if we were to put a key/value pair \jsinline|(k, v)| into the map, then 
we should be able to retrieve the value \jsinline|v| using the key \jsinline|k|. 
Without \jilette we  would have to pick a concrete key and a concrete value. 
Instead, we can simply use a symbolic string for the key and a symbolic number 
...
When running \jilette on the above test with a naive implementation  
we obtain the counter-model  \jsinline|s1 = "hasOwnProperty"|. This is due to the 
fact Note that one can insert a key-value pair with \jsinline|"hasOwnProperty"| as a key into the map. 
By doing this, \jsinline|"hasOwnProperty"| in the prototype chain of
 ...
 We can run \jilette on this test to reveal the bug discusses above. Indeed, \jilette generates
