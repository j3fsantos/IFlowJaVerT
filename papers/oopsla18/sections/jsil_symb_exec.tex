...
This methodology consists of the following steps: \dtag{1} designing an instrumented version
of the language semantics that exhibits the frame property, \dtag{2} designing the program 
analysis on the instrumented semantics, and \dtag{3} linking the program analysis 
...
In the spirit of~\cite{vanhorn:icfp:2010}, we define a parameterisable semantics for \jsil, 
which can be instantiated with a \jsil state signature. The parameterisable semantics is the bedrock 
for the formal development and implementation of our symbolic analysis. In fact, we use 
it to obtain the concrete, instrumented, and symbolic semantics, by instantiating it 
with a concrete, instrumented, and symbolic state signature, respectively. 
This approach to the design of symbolic analyses has the following benefits: 
\dtag{1} it streamlines the formalism, avoiding redundancy,
\dtag{2} it makes the choices in the design of the instrumented and symbolic semantics explicit,
...
\subsection{Parameterisable Semantics}\label{subsec:jsil:analysis:formalism}
...
\subsubsection{Parameterisable Semantics}
...
\setlength{\itemsep}{0.2em}
  ...
  \item[GetDomain,] $\getdomain \subseteq \absstates \rightarrow \exprs \partialmap \absvals$. 
   ...
   \item[Assumption,] $\absassume{} : \absstates \rightarrow (\exprs \partialmap \absvals) \partialmap \absstates$. 
            ...
            abstract state $\absstate$ when it holds that $\jsilexpr$ evaluates to the boolean $\jtrue$. 
...
\begin{mathpar} 
 ...
 \qquad
   ...
   {\val \in \vals \text{ is of type } \jtype}{\absmakesymbolicrule{\concrete}{\jstate, \jtype} = \val}
     \and 
  \inferrule[\textsc{Assume}]
   {
       \evalexpr{\concrete}(\jstate.\stosel, \jsilexpr) = \jtrue
   }{  \absassume{\concrete}(\jstate, \jsilexpr) =  \jstate }
  	\and
    \inferrule[\textsc{Check Sat}]
   {
        \jbool = \evalexpr{\concrete}(\jstate.\stosel, \jsilexpr) 
   }{  \abssat{\concrete}(\jstate, \jsilexpr) = \jbool}
  \end{mathpar}
  }}
 \end{display}
