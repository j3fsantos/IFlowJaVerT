...
\quad
 \semexpr{(\pid, \sstore, \jvar, i, j) \lstcons \scs}{\senv} \semeq (\pid, \semexpr{\sstore}{\senv}, \jvar, i, j) \lstcons \semexpr{\scs}{\senv}}
 \\
 %
\inferrule[Symbolic Heaps]{}{
\interpret{\symbolic}(\senv, \hemp) \semeq \hemp, \emptyset
\quad
\frac{
   l = \semexpr{\sexprl}{\senv} \quad
    p = \semexpr{\sexprp}{\senv} \quad v = \semexpr{\sexprp}{\senv}
}{
\interpret{\symbolic}(\senv, \hcell{\sexprl}{\sexprp}{\sexprv}) \semeq  \hcell{l}{p}{v}, \emptyset
}
\qquad
\frac{
   l = \semexpr{\sexprl}{\senv} \quad p = \semexpr{\sexprp}{\senv}
}{
  \interpret{\symbolic}(\senv, \hcell{\sexprl}{\sexprp}{\none}) \semeq \hemp, \jsilset{ (l, p) }
}
\qquad
\frac{
   \interpret{\symbolic}(\senv, \sheap_i) = h_i, \hdom_i, i =1,2
}{
\interpret{\symbolic}(\senv, \sheap_1 \dunion \sheap_2) \semeq  h_1 \dunion h_2, \hdom_1 \dunion \hdom_2
}}
\\
\inferrule[Symbolic Domains]
{}{
\interpret{\symbolic}(\senv, \sdom) 
    \semeq \{ (l, p) \mid \sexprl \in \domain(\sdom) \, \wedge \, l = \semexpr{\sexprl}{\senv} \, \wedge \, p \not\in \semexpr{\sdom(\sexprl)}{\senv} \}
}
\and
\inferrule[Symbolic CF-States]{
    \sstate = (\sheap, \sdom, \sstore, \pc) 
    \quad 
    \interpret{\symbolic}(\senv, \sheap) = h, \hdom_1
   \quad
    \interpret{\symbolic}(\senv, \sdom) = \hdom_2
    \quad
...
\jmodels{\symbolic}(\sstate, \scs) \semeq \{ ((h \dunion h', \store), \cs) \mid \domain(h') \cap \hdom = \emptyset \} 
}
\end{mathpar}}
\end{display}
...
$$
\begin{array}{l}
\transabssemrule{\sstate, \scs, i}{\sstate', \scs', j}{\mode}{\mode'}{\symbolic}  \ \wedge \ (\jstate, \cs) \in \jmodels{\symbolic}(\sstate[\pcsel \mapsto \sstate'.\pcsel], \scs) \\ \quad \quad 
    \ \implies \ \exists \jstate', \cs' \, . \, 
        \transabssemrule{\jstate, \cs, i}{\jstate', \cs', j}{\mode}{\mode'}{\concrete} \ \wedge \ (\jstate', \cs') \in \jmodels{\symbolic}(\sstate', \scs')
\end{array}
$$
\end{theorem}
...
\wedge_{k=1}^n \, \big( \transabssemrule{\sstate, \scs, i}{\sstate_k, \scs_k, j_k}{\mode}{\mode_k}{\symbolic}  \big)
    ...
    \, \wedge \, (\jstate, \cs) \in \jmodels{\symbolic}(\sstate, \scs)
    \\ \quad \quad
%
      \ \implies \ \exists 1 \leq l \leq n, \jstate', \cs' \, . \, 
           \transabssemrule{\jstate, \cs, i}{\jstate', \cs', j_l}{\mode}{\mode_l}{\concrete}
           \, \wedge \, 
           (\jstate', \cs') \in \jmodels{\symbolic}(\sstate_l, \scs_l)
\end{array}
$$
\end{corollary}
...
\qquad
\frac{
      l = \semexpr{\sexprl}{\senv} \quad p = \semexpr{\sexprp}{\senv} \quad 
      v = \left\{ {\begin{array}{ll} 
          \semexpr{\sexprp}{\senv} & \text{if } \sexprv_{\none} \neq \none \\ 
          \none                                & \text{otherwise} 
      \end{array}}\right.
}{
\interpret{\symbinst}(\senv, \hcell{\sexprl}{\sexprp}{\sexprv_{\none}}) \semeq  \hcell{l}{p}{v}
}
\qquad 
\frac{
   \interpret{\symbinst}(\senv, \sheap_i) = \iheap_i, i=1,2
}{
\interpret{\symbinst}(\senv, \sheap_1 \dunion \sheap_2) \semeq  \iheap_1 \dunion \iheap_2
}}
\\
\inferrule[Symbolic Domains]
{
...
\jmodels{\symbinst}(\sstate, \scs) \semeq ((h, \idom, \store), \cs)
}
\end{mathpar}}
\end{display}

\begin{lemma}[Bounded-Soundness - Instrumented Semantics]\label{teo:soundness:jsil:symb:exe:instrumented:instrumented}
$$
\begin{array}{l}
...
\begin{mathpar}
\inferrule[Symbolic Heaps]{}{
\interpret{\instrumented}(\senv, \hemp) \semeq \hemp, \emptyset
\quad
\interpret{\instrumented}(\senv, \hcell{\loc}{p}{\val}) \semeq  \hcell{\loc}{p}{\val}, \emptyset
\qquad
  \interpret{\instrumented}(\senv, \hcell{\loc}{p}{\none}) \semeq \hemp, \jsilset{ (l, p) }
\qquad
\frac{
   \interpret{\instrumented}(\senv, \iheap_i) = h_i, \hdom_i, i =1,2
}{
\interpret{\instrumented}(\senv, \iheap_1 \dunion \iheap_2) \semeq  h_1 \dunion h_2, \hdom_1 \dunion \hdom_2
}}
\\
