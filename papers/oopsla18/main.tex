...
In \S\ref{???}, we give a novel abstract semantics of \jsil, which we instantiate to obtain the concrete and symbolic semantics, connected via a {\em soundiness result}. Moreover, we prove that \jilette is precise, meaning that it never produces incorrect bug reports. 
...
Compositionality also allows us to state a verification result for \jilette, precisely capturing the conditions under which our symbolic execution is sound.
To our knowledge, \jilette is the first compositional symbolic execution tool for dynamic languages.
%
