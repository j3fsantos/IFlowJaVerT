...
However, due to its dynamic nature and complex semantics, JavaScript remains a difficult target for symbolic analysis and logic-based verification.

\pmax{Now, sth about the state-of-the-art}

There exist symbolic execution tools for JavaScript, such as \pmaxinline{list them, cite them}. \pmaxinline{Say also something positive, don't know what}. They are not formalised and they do not come with guarantees wrt semantics \pmaxinline{check}. They are not compositional \pmaxinline{We don't know what this means yet}. Each of them targets a specific type of bugs \pmaxinline{give evidence}. 

\pmax{How do we fill the gap?}
To address this issue, we present \jilette, a framework for symbolic execution of JavaScript (ECMAScript 5 Strict~\cite{ecma}) programs, which advances the state-of-the-art in the two following ways. 

\myparagraph{Novelty: Trustworthiness} The symbolic execution of \jilette is \emph{trustworthy}, in that it is formalised, proven sound, and precisely follows the ES5 English standard. 
This symbolic execution is not done directly on JavaScript, but instead on \jsil, an intermediate representation previously introduced for JavaScript verification~\cite{javert}. 
...
These theoretical results, combined with the correctness of \JSComp, allows us to lift the results of analyses done on compiled \jsil code back up to JavaScript. To our knowledge, this is the first formalisation of a symbolic execution used for JavaScript analysis, and is the first symbolic execution that precisely follows the semantics of the language.

\myparagraph{Novelty: Compositionality} 
Unlike C, C++, or Java, JavaScript and \jsil do not observe the frame property~\cite{???}. This means that the behaviour of a JS/\jsil program may change if the state in which it is run is extended; such extensions may introduce bugs, to which we refer as \emph{frame bugs}. 
%
Current symbolic execution tools for JavaScript~\cite{???} employ \emph{whole-program analyses}, that is, they assume access to the entire program. This means that the results they obtain when analysing functions in isolation cannot be reused, as they would not account for the interaction between the function and all of its possible frames.
%
In contrast, the symbolic execution of \jilette is \emph{compositional}. \jilette can analyse a fragment of a program at a time, and the obtained results can be re-used in the analysis of the entire program.
%
Having a compositional symbolic execution has two important benefits.
%
...
%
We achieve compositionality by instrumenting the concrete and symbolic semantics to keep track of properties that we know are {\em not present} in a given object. This we describe in detail in \S\ref{???}. To our knowledge, \jilette is the first compositional symbolic execution tool for dynamic languages.
...
We illustrate the capabilities of \jilette on an array of examples, which include tests from the Test262 test suite and real-world Node.js libraries, and which make use of JavaScript-specific features, such as prototype inheritance, function closures, the for-in statement, and dynamic dispatch. 

\myparagraph{Application: Specification-directed Bug-finding} Due to the complexity of JavaScript semantics, functional correctness 
