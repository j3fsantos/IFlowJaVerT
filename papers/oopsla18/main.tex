...
We prove that the symbolic execution underpinning \cosette is sound and that it does not generate false positives. Moreover, this symbolic execution is compositional, which allows us to use \cosette to find bugs in separation logic specifications of JavaScript programs that are not detectable by standard symbolic execution techniques.
...
%%symbolic execution is theis....

Symbolic execution is a software testing technique that
has been successfully used to analyse code in languages such as C,
Java and JavaScript.  


Symbolic execution uses mathematical logic to
reason about program behaviour, providing much stronger guarantees
than traditional concrete testing since the execution of program paths is
considered in the context of all possible inputs. It has recently been
shown to be practical as a result of significant advances in
fundamental theory, solver technology, and scalable tool development.

Symbolic execution is a whole-program testing technique that
demonstrates
that a program behaves well given  an initial context. The technique  works
well for well-behaved languages such as C and Java that satisfy the
{\em frame property}. Intuitively, the frame property means that if a
program produces a given output when run in a given state, then we can
extend this state arbitrarily, run the program in this extended state,
and the program will produce the same output. With the frame property,
symbolic testing with respect to one context gives some assurance that 
the program works well with respect to all contexts. It gives
assurance that
...
server-side scripting via 
Node.js; and it is even run on
small embedded devices with limited memory. It is used by 94.8\% of websites , and is the most
active language in both GitHub and StackOverflow. 
...
Standardised by the
ECMAScript committee and natively supported by all major browsers, JavaScript is a complex and evolving language. 



we are analysing JS using symbolic execution

symbolic execution, lots of success, concrete unit tests to symbolic
tests. 
...
\myparagraph{Novelty: Trustworthiness} Designing a tractable symbolic analysis for JavaScript that is faithful to the language standard and providing correctness results for such an analysis is known to be a difficult task, due to the complexity of the language. The current approaches~\cite{.,.,.} normally do not give a formal account of the analysis and/or simplify the language semantics. Our approach to designing \cosette is grounded on adherence to the standard and establishing trust. 
...
In \S\ref{???}, we give a novel abstract semantics of \jsil, which we instantiate to obtain the concrete and symbolic semantics, connected via a {\em soundiness result}. Moreover, we prove that \cosette is precise, meaning that it never produces incorrect bug reports. 
...
In \S\ref{???}, we show how to use \cosette
...
\myparagraph{Application: Specification-directed Bug-finding} \pmaxinline{Feeling stuck here.} Due to the complexity of JavaScript semantics, functional correctness 
...
the developer needs to understand in detail a complicated proof trace (\javert), or  act with essentially no feedback~(KJS). In \S\ref{???}, we show how \cosette can be used as an auxiliary mechanism for debugging 
...
\newpage
\section{Motivational Examples (erm, this really looks like an overview)}\label{sec:overview}
