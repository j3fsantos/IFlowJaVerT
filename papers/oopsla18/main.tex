...
\begin{abstract}
We present \jilette, a framework for trusted, compositional symbolic execution of JavaScript programs.
...
In current program analysis research, there is a gulf between bug-finding techniques and verification techniques. While bug-finding techniques aim at finding concrete executions that cause the input program to fail, verification techniques aim at proving that the input program will never fail for the inputs satisfying its preconditions. The former emphasise precision, whereas the latter emphasise soundness. Ideally, one would always go for soundness. However, soundness comes at a cost: verification techniques are often too conservative, rejecting many programs that are in fact sound, and also have prohibitive scalability issues. Furthermore, there is a growing consensus that soundness is not even necessary for most modern analysis applications, as many clients can tolerate unsoundness (auto-complete systems, code navigation etc). In [], the authors propose soundiness instead of soundness as the de facto standard. On the other hand, bug-finding tools are also lacking in that they are usually not justified with respect to the semantics of the targeted language, often relying on unstated simplifying assumptions, when not explicitly departing from the semantics of the language. As a result, the fact that a bug finding tool cannot find a bug gives no formal guarantee with respect the correctness of the input program. We trust these tools because they appear to work empirically. Another difference between bug-finding and verification techniques is that bug-finding tools are often underpinned by a whole-program analysis, whereas verification techniques are normally compositional, meaning that the results of the analysis for a given program can be re-used when analysing a program that contains it. We believe that compositionality is essential for tools that need to run regularly on large code bases. For instance, it would not be possible for Infer, a separation logic based bug-finding tool, to be part of the continuous integration system at Facebook, if it were not compositional. However, compositionality is not trivial to achieve when the targeted programming language does not exhibit the frame property, as is the case for most dynamic languages including JavaScript.  
This paper aims at the sweet spot between bug-finding and verification. It presents a compositional symbolic execution for JavaScript whose soundiness guarantees are formally characterised. 
...
We present \jilette, a framework for symbolic execution of JavaScript (ECMAScript 5 Strict~\cite{ecma}) programs, which advances the state-of-the-art in the two following ways. First, the symbolic execution of \jilette is \emph{trustworthy}, in that it is formalised, proven sound, and precisely follows the ES5 English standard. This symbolic execution is not done directly on JavaScript, but instead on \jsil, an intermediate representation previously introduced for JavaScript verification~\cite{javert}. \jsil comes with a trusted infrastructure that allows us to provably lift the results of our symbolic analyses on compiled \jsil code back up to JavaScript. Second, the symbolic execution of \jilette is \emph{compositional}, allowing us to use \jilette to debug specifications of JavaScript programs and catch a broader class of bugs than the current symbolic execution tools.
