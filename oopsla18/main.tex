...
\begin{abstract}
We present \jilette, a framework for trusted, compositional symbolic execution of JavaScript programs. \jilette is the first symbolic analysis tool for JavaScript that precisely models the semantics of the language and is, more generally, the first tool that targets dynamic languages whose symbolic execution is compositional. It is aimed at the general programmer, allowing them to debug their code by writing symbolic tests for which \jilette then produces concrete counter-models. The symbolic execution of \jilette is proven to be sound and not to generate false positives. 
%
%\pmaxinline{Now compositionality.}
%
We evaluate \jilette on an array of examples, including tests from the official JavaScript Test262 test suite and real-world Node.js libraries. These examples involve JavaScript-specific features, such as prototype inheritance, function closures, the for-in statement, and dynamic dispatch. We highlight the range of \jilette by showing how it can be used to produce counter-models for separation logic specifications of JavaScript programs.
%
...
\item 
	...
	{\bfseries Novelty}: The precision. \\ 
...
\item 
	...
	{\bfseries Novelty}: Unclear. \\ 
	{\bfseries Evaluation}: Tests for the symbolic execution rules; JS programs using prototype inheritance, function closures, for-in, dynamic dispatch, etc.; test262 tests; node.js libraries for data structures
