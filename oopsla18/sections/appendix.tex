...
\inferrule[\textsc{Skip}]{}
	{ \sembtransbasic{\jstate, \jsilskip}{\jstate}{\top}} 
\and 
...
\inferrule[\textsc{Object Creation}]
  { 
   \heap = \jstate.\hpsel
    ...
    \quad 
    \store' = \store[\jvar \mapsto \loc] 
  }{\sembtransbasic{\jstate, \jvar := \jsilnew()}{\jstate[\hpsel \mapsto \heap']}}
\and 
...
\inferrule[\textsc{Member Check - True}]
  { 
      \store =  \jstate.\stosel 
      \quad
      \getcellrule{\jstate, (\jsilexpr_1, \jsilexpr_2)}{(\loc, p, \val)} 
      \\\\
      \val \neq \none
      \quad 
...
\end{mathpar}}
\vspace*{-0.6cm}
\caption{Execution for Basic Commands: {\scriptsize$\sembtransbasic{\jstate, \bcmd}{\jstate'}{\mode}$}\label{fig:app:sem:basic:commands}}
\end{figure}
...
\inferrule[\textsc{Basic Command}]
   ...
   }{\semtrans[\prog][\top][\top]{\jstate, i}{\jstate', i+1}[C]}
  %
  \and
  %
  \inferrule[\textsc{Cond. Goto - True}]
   { \ccmd[\prog][\ctx]{i} =  \ifgoto{\jsilexpr}{j}{k} 
     \\\\
     \symbeval{\jsilexpr}{\jstate.\stosel} =  \jtrue
   }
   {\semtrans[\prog][\top]{\jstate, i}{\jstate, j}[C]}
  %
  \and 
  % 
  \inferrule[\textsc{Cond. Goto - False}]
   ...
   {\semtrans[\prog][\top]{\jstate, i}{\jstate, k}[C]}
  %
  \\
  %
  \inferrule[\textsc{Goto}]
   ...
   {\semtrans[\prog][\top]{\jstate, \procretlab}{ i}[C][C']}
     %
   \qquad 
   %
...
% \pmax{What happens when we exit from main, how do we stop? Basically, cmd returns nothing and we can't reduce?}
 \vspace*{-0.6cm}
\caption{Execution for Control Flow Commands: $\semtrans[\prog][\mode][\mode']{\jstate, i}{\jstate', j}[C][C']$}
\end{figure}
%
