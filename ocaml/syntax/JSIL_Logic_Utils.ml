...
let rec type_lexpr gamma le =
  ...
  | LLit lit -> (Some (evaluate_type_of lit), true, [])
	...
	| LVar var
	| PVar var ->
		(match Hashtbl.find gamma var with
		| Some t -> Some t, true, []
		| None   -> None,   true, [])
...
let rec reverse_type_lexpr_aux gamma new_gamma le le_type =
  ...
  | LLit lit -> if (types_leq (evaluate_type_of lit) le_type) then true else false
	...
	| LVar var
	| PVar var ->
		(match (Hashtbl.find gamma var), (Hashtbl.find new_gamma var) with
		| Some t, None
		| None, Some t     -> if (types_leq t le_type) then true else false
		| None, None       -> (Hashtbl.replace new_gamma var (Some le_type)); true
...
let reverse_type_lexpr gamma le le_type : ((string, jsil_type option) Hashtbl.t) option =
