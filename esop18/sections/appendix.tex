...
\begin{lemma}[Soundess of \jsil symbolic execution - Single Step]\label{lemma:soundness:single:step}
$$
\begin{array}{l}
\symbtrans{\sheap, \sstore, i, \pc}{\sheap', \sstore', j, \pc'}[\sctx][\sctx'] 
   \ \wedge \ 
...
%
\begin{proof}
We proceed by case analysis on $\symbtrans{\sheap, \sstore, i, \pc}{\sheap', \sstore', j, \pc'}[\ctx][\ctx']$. 
\vspace{5pt}
...
$$
\symbtrans{\sheap, \sstore, i, \pc}{\sheap'', \sstore'', k, \pc''}[\sctx][\sctx''] (\ieq{1})
\qquad 
\symbtranstrans{\sheap'', \sstore'', k, \pc''}{\sheap', \sstore', j, \pc'}[\sctx''][\sctx'] (\ieq{2})
$$
Applying Lemma~\ref{lemma:pc:monotonocity} to \ieq{2}, we conclude that 
$\pc' \vdash \pc''$ (\ieq{3}). From \ieq{3} and \hyp{2}, we conclude that 
$(\heap, \store, \ctx) \in \smodels{\sheap, \sstore, \sctx}{\pc''}$ (\ieq{4}).  
Applying Lemma~\ref{lemma:soundness:single:step} to \ieq{1} and \ieq{4}, 
we conclude that there is a heap $\heap''$, store $\store''$, and 
context $\ctx''$, such that: 
$$
 \semtrans{\heap, \store, i}{\heap'', \store'', k}[\ctx][\ctx'']
\, \wedge \, 
(\heap'', \store'', \ctx'') \in \smodels{\sheap'', \sstore'', \sctx''}{\pc''}  
