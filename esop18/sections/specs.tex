...
for generating symbolic tests from separation logic specifications, so that if a 
...
\end{mathpar}}
\hspace{-3pt}Determining whether or not a symbolic state satisfies a SL-assertion $P$ is, in general, 
...
\dtag{2} a set $\cells$ containing the non-none cell assertions (those whose value is different from $\none$), 
...
\inferrule[\textsc{Extra Resource -  Fail}]
	{  
	    \sheap \neq \hemp
	}{\unificationfull{\sheap, \sstore, \pc}{\emptyset, \pfs'}{\subst}} 
%

\end{mathpar}}
\hrule
\caption{Unification algorithm: {\small $\unificationfull{\hemp, \sstore, \pc}{\emptyset, \pfs'}{\subst}$}
and {\small $\unificationfullfail{\sheap, \sstore, \pc}{\sfs, \pfs'}{\subst}{\pc'}$}.\label{unification:algorithm}}
\end{figure}
...
$(\sheap, \sstore, \pc)$ satisfies $P$. 
The bug-finding algorithm is more subtle. It states that: if for all possible substitutions 
...
\quad \forall_{0 \leq k \leq n} \, . \, \unificationfullfail{\sheap, \sstore, \pc}{\cells, \pfs'}{\subst}{\pc_k} 
%
