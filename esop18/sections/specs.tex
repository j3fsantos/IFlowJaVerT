...
we represent each \polish{possible mapping from the existentially quantified symbolic locations in $P$ 
...
\inferrule[\textsc{Cell Assertion}]
	{  
	   \subst(\symbeval{\lexpr_l}{\sstore})  = \loc 
	   \quad 
	    \polish{\subst(\symbeval{\lexpr_p}{\sstore}) = \sexprp'} 
	   \quad
	    \polish{\subst(\symbeval{\lexpr_v}{\sstore}) = \sexprv'} 
	   \quad
	    \sheap = \sheap_f \dunion ((l, \sexprp) \mapsto \sexprv) 
	   \quad
	    \pc \vdash  \polish{\sexprp = \sexprp' \wedge \sexprv = \sexprv'}
	...
	{  
	  \subst(\symbeval{\lexpr_l}{\sstore}) = \loc
	   ...
	   \forall_{0 \leq i \leq n}. \, 
		...
		\uno{\wedge_{0 \leq i \leq n} (\polish{\pc_i} \vee \polish{\pc_i'})}} 
\end{mathpar}}
\end{display}
...
\inferrule[\textsc{Iterated Cell Unification}]
	{  
	   \unification{\sheap, \sstore, \pc}{(\lexpr_l,\lexpr_p)\pointsto \lexpr_v}{\subst} = \uyes{\sheap_f}
	}{\cellunification{\sheap, \polish{\cell} \lstcons \cells}{\polish{\sheap_q}, \cells}{\sstore, \pc, \subst}} 
\and
