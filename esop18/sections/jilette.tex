...
This code: 
	...
	3)~asserts that the length of the list in $\mathtt{x}$ is 2, i.e.~that~$\mathtt{o}$ has two properties in the end. This last assertion will cause the symbolic execution to fail. Let us understand why.
...
The theorem of soundness (Theorem~\ref{teo:soundness:jsil:symb:exe}) states that if we have a symbolic trace captured by 
...
$\semtranstrans{\heap, \store, i}{\heap', \store', i'}[\ctx][\ctx']$. 
 We use the final path condition $\pc'$ for both the models of initial and final 
...
failing assertion. As \jilette is meant to be a \emph{bug-finding} tool, it is essential 
not to generate false positives, only reporting to the user errors that do in fact  exist. 
%
\begin{corollary}[Bug-finding]\label{bug:finding}
$$
\begin{array}{l}
\forall \, \sheap, \sstore, \sctx, i, \pc, \pc' \, . \,  
 \symbtranstranserr{\sheap, \sstore, i, \pc}{\sctx}{\pc'}   \implies 
...
then all concrete states in the models of the initial symbolic state (under the 
initial path condition) will result in final concrete states in the models of 
one of the final symbolic states (under its associated path condition).  
Clearly, if a \jsil program contains loops that are not possible to unroll 
statically, we will never be in the case of the verification corollary. 
%
...
\end{corollary}
%
The proofs of all the results are given in the Appendix. 
