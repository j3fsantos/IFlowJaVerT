...
The ES5 standard spans over more than 200 pages describing the language syntax, semantics, 
...
\jilette is a symbolic analysis for ES5 Strict that works by first compiling ES5 Strict programs to \jsil 
using the \JSComp compiler. 
...
ES5 built-in libraries on the fly.  
These implementations are both an easy way to get a better coverage of the standard and 
make sure that we make proper use of the solver-aided facilities of Rosette.    
...
\polish{The punchline is that the unary and binary operators in the assume and assert 
constructs are not the JavaScript ones but the \jsil ones with a very clear and 
simple semantics (no coercions! no side-effects! nothing!).}
