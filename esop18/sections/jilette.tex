...
\end{lstjsex}
%
The symbolic test above checks for a desired property of the library---if we were to put a key/value pair \jsinline|(k, v)| into the map, then we should be able to retrieve the value \jsinline|v| using the key \jsinline|k|. We can run \jilette on this test to reveal the bug. Indeed, \jilette generates
...
Below, we show the \rosette model for \jsinline|String.replace| (c.f. Ch.15.5.4.11~\cite{ecma}).
...
all the occurrences \emph{script tags} inside untrusted strings (for instance, those coming 
from user input or untrusted servers).\footnote{
Despite its simplicity, this example illustrates the complexities underpinning the 
design and implementation of robust string sanitisers,  which are a commonly  
used defense mechanism against XSS attacks~\cite{song}. 
}
To this end, the programmer chooses to 
replace the all occurrences of the the string \jsinline|"script"| in the string given 
as input with \jsinline|"s"|. 
\begin{lstjsex}
function sanitise (str) { return str.replace("script", "s") }
\end{lstjsex}
Clearly, this program does not achieve what it is meant to do, since one can 
give it the input \jsinline|"scriptcript"|. 
Consider the following test for the sanitiser function above. 
\begin{lstjsex}
var s = sanitise(_s()); 
var x = !(s.contains("script")); 
assert(x) 
\end{lstjsex}
The symbolic test above checks for a desired property of the function \jsinline|sanitise|---if we
were to sanitise a string, then the obtained string should not contain any occurrence of 
\jsinline|"script"|. We can run Jilette on this test to reveal the bug. Indeed, Jilette 
generates the failing model: \jsinline|s1 = "scriptcript"|. 
