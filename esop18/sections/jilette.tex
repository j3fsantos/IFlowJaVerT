...
\pmax{If I understood correctly, 2,3,4,5 are all related to the procedure that called $f$. Not to overload, the $i$ and $j$ should probably be different, like $r_n$ and $r_e$ or whatever. What is the initial context when executing top-level code, which is not called by anything? $C$ is not affected by anything, and the $i$ is not substituted anywhere inside, it should be out of the triple and implicit}
...
\pmax{How is the path condition used? Should it be an ordered list? It feels like a loss of information that we can have same path conditions for different branchings. No, not really, since this is not just gotos, it's a lot more than that.
}
