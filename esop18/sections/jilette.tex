...
As \jilette does not infer loop invariants, if a \jsil program contains loops that are 
not possible to unroll statically, we will never be in the case of the verification corollary. 
...
\end{corollary}
%
The {\bf proofs} of the results above are given in the Appendix. 
...
%
%Since it is possible for more than one transition of the symbolic 
%semantics to be applicable, the symbolic execution may 
%give rise to an intractable number of possible symbolic states. 
%
%Keeping this number under control requires the use of sophisticated merging algorithms. 
% 
...
\begin{figure}[t!]
\begin{lstlisting}
(define (mutate-pv-list pv-list prop new-val)
  (cond
    [(null? pv-list)
     (list (cons prop new-val))]
    [(equal? (car (car pv-list)) prop)
     (cons (cons prop new-val) (cdr pv-list))]
    [ else (cons (car pv-list) 
              (mutate-pv-list (cdr pv-list) prop new-val))]))
...
(define (mutate-heap heap loc prop val)
  (cond
    ...
    [(equal? (car (car heap)) loc)
     (cons (cons loc (mutate-pv-list (cdar heap) prop val)) (cdr heap))]
    [ else (cons (car heap) (mutate-heap (cdr heap) loc prop val))]))
  ...
  (let ((cmd-type (first bcmd)))
    (cond
    	[(eq? cmd-type 'h-assign)
         ...
         ...)))
\end{lstlisting}
\caption{Fragment of \jsil Interpreter in \rosette}\label{rosette:interpreter:fragment}
\end{figure}
