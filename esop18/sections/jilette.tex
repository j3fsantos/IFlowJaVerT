...
\jsil commands are represented in \rosette as \emph{s-expressions}. 
 ...
 is represented as: 
$$
(\racketlist \ (\racketquote \text{p-assign}) \ \roscomp{\jsilexpr_1} \ \roscomp{\jsilexpr_2} \ \roscomp{\jsilexpr_3})
$$
Hence, the interpreter of basic commands first checks if the first element of the 
...
The function \schemeinline|update-heap| recursively iterates over all the objects
in the heap to find the object whose property is to be updated and, when it finds it, 
uses the function \schemeinline|update-pv-list| to update the corresponding 
field-value list. If it does not find it, it simply creates a new object with that property. 
