...
\begin{tabular}{lllll}
	 Numbers: $\jnumber \in \numbers$ & \quad Booleans: $\jbool \in \bools$ & \quad \ Strings: $\jstring \in \strings$ & \quad \ Locs: $\loc \in \locs$ & \quad Vars: $\xivar \in \jvars$ \\[0.1cm]
	Types: $\ivltype \in \ivltypes$ & \multicolumn{4}{l}{\quad Literals: $\lit \in \lits$ \defeq \ $\jnumber \mid \jbool \mid \jstring \mid  \jsundefined \mid \jsnull \mid \loc \mid \ivltype \mid \fid \mid \jsilempty$} \\[0.1cm]
\multicolumn{5}{l}{Expressions: $\jsilexpr \in \exprs$ \defeq \ $\lit \mid \xivar \mid \ominus\ \jsilexpr \mid \jsilexpr \binop{} \jsilexpr $} \\[0.1cm]
	\multicolumn{5}{l}{Basic Commands: $\bcmd \in \bcmds$ \defeq\ $\jsilskip \mid \xivar := \jsilexpr  \mid \xivar := \jsilnew() \mid \xivar := [\jsilexpr, \jsilexpr] \mid [\jsilexpr, \jsilexpr] := \jsilexpr \mid$} \\[0.1cm]
	\multicolumn{5}{l}{\hspace{1.35cm} $\jsildelete(\jsilexpr, \jsilexpr) \mid \xivar := \hasfield(\jsilexpr, \jsilexpr) \mid \xivar := \getfields(\jsilexpr) \mid \assume(\jsilexpr) \mid \assert(\jsilexpr)$} \\[0.1cm]
	% Commands
	\multicolumn{5}{l}{Commands: $\ivlcmd \in \cmds$ \defeq \ $ \bcmd \mid \goto \ i \mid  \ifgoto{\jsilexpr}{i}{j} \mid \jsilcall{\xivar}{\jsilexpr}{\jvec{\jsilexpr}}{j} \mid \xivar := \phi(\overline{\xivar})$} 
 \end{tabular} \\[0.1cm]
 \begin{tabular}{rcrclcl}
 	Procedures & : & $\proc$ & $\in$ & $\procset$ &\defeq& $\procedure{\fid}{\jvec{\xivar}}{\jvec{\ivlcmd}}$ \\[0.1cm]
	%Notation & : & \multicolumn{5}{l}{$\jvec{\xivar}$,  $\jlist{\lit}$,  $\overline{\jsilexpr}$, and $\overline{\ivlcmd}$, respectively, denote lists of variables, literals, expressions, and commands. } && \multicolumn{5}{l}{$\set{\lit}$ denotes a set of literals.}
 \end{tabular}
 \end{minipage}
 \end{display}
 
%Most syntactic constructs of JSIL either come directly from JavaScript or are useful for JavaScript analysis. 
\jsil literals, $\lit \in \lits$, include numbers, booleans, strings, the special values $\jsundefined$ and $\jsnull$, as well as types~$\ivltype$, procedure identifiers $\fid$, and the special value $\jsilempty$. 
\jsil~expressions, $\jsilexpr \in \exprs$, include \jsil literals, \jsil program variables $\xivar$, and various of unary and binary operators, which, for instance, provide support for sets and lists. 
...
\jsil commands include basic \jsil commands and a number of commands related to control flow: conditional gotos, unconditional gotos and dynamic procedure calls.\footnote{\jsil also has $\phi$-node commands, which allow the \JSComp compiler to produce code directly in Static-Single-Assignment (SSA) \cite{SSA}. To avoid clutter, we omit the $\phi$-nodes from this formalisation; they do not impact the reasoning in any way.} 
...
The dynamic procedure call $\jsilcall{\xivar}{\jsilexpr}{\jvec{\jsilexpr}}{j}$ first evaluates  $\jsilexpr$ and $\jvec{\jsilexpr}$ to obtain the procedure name and arguments, respectively, executes the appropriate procedure with these arguments, and, in the end, assigns its return value to $\xivar$.
If the procedure raises an error, control is transferred to the $j$-th command, and to the next otherwise. 

A \jsil program $\prog \in \ivlprogs$ can be seen as a set of top-level procedures $\procedure{\fid}{\jvec{\xivar}}{\jvec{\ivlcmd}}$, where $\fid$ is the procedure name , $\jvec{\xivar}$ its formal parameters, and its body ${\jvec{\ivlcmd}}$  is a \emph{command list} consisting of a numbered sequence of \jsil commands.
Every \jsil program contains a special procedure $\jsilmain$, which denotes the entry point of the program. 
\jsil procedures return via two dedicated indexes, $\procretlab$ and $\procerrlab$. If a procedure reaches the $\procretlab$ index, it returns normally with the return value $\procretvar$; when it reaches $\procerrlab$, it returns an error, with the error value $\procerrvar$
