...
The execution then continues in both branches with the straightforward field collection command $\mathtt{x := getFields(o)}$, which assigns the list of properties of the object $\mathtt{o}$ to variable $\mathtt{x}$. Finally, we execute $\mathtt{assert\ (llen \ x == 2)}$, asserting that $\mathtt{o}$ has exactly two properties, which holds in the right branch, but fails in the left. 
