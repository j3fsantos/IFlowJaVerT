...
\begin{abstract}
We present \jilette: a symbolic execution tool for ES5 Strict. Jilette supports 
string reasoning and precisely follows the language standard, leveraging
on \underline{\JSComp}, a well-tested compiler from ES5 Strict to an 
intermediate language well-suited for verification and analysis; and \underline{Rosette},
a solver-aided host language developed to facilitate the design of new 
solver-aided domain-specific languages. 
We study two complementary uses of \jilette. 
First, we show how \jilette can be used for symbolic testing of JavaScript programs. 
Second, building on \jilette, we develop a tool for debugging separation logic specifications
by compiling them to symbolic tests in order to find bugs in both specifications 
and code. 
\end{abstract}
...
This paper presents \jilette, a symbolic execution tool for ES5 Strict. Jilette has the following benefits: \dtag{1} it is grounded on the real semantics of the language and does not simplify the memory model in any way; \dtag{2} it has extensive coverage, supporting the entire core language and almost all built-in libraries; \dtag{3} it supports advanced string reasoning, critical for reasoning about \polish{everyday} JavaScript code; \dtag{4} it provides \polish{intuitive assertion/debugging mechanisms} accessible to the general JavaScript programmer, with minimal annotation overhead.
%
We highlight two possible, relevant use cases for \jilette. First, we show how \jilette can be used as \dtag{i}~a tool for running symbolic tests for JavaScript programs; and \dtag{ii} a debugging tool for separation logic specifications of JavaScript programs. \pmaxinline{Can we be more general, and say something like 'logic-based specifications'? It's all about translating to FOL. Also, we need to say at some point why we care about specifications written in separation logic.}
