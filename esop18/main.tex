...
\dtag{2} \polish{it is provably sound, guaranteeing no false positives};
...
The core of \jilette consists of a sound symbolic interpreter for
\jsil~\cite{javert}, an intermediate goto language well-suited 
for verification. We obtain this symbolic interpreter \emph{for free}, 
by implementing a concrete \jsil interpreter in Rosette~\cite{Rosette2,Rosette1}, 
a symbolic virtual machine that facilitates generation of solver-aided languages.
...
of their program. 
% it's not really js2jsil - it is the extensible symbolic interpreter
% This we fix when you get here
Moreover, we adjust \JSComp so that the abstraction level 
...
\polish{Some people want FCSpecs of JS. They are bananas.}
Much like the semantics of JavaScript, functional correctness 
specifications of JavaScript programs are highly intricate. 
There are only a few tools (for example, \javert \cite{javert} and KJS \cite{kjs}) with support for such expressivity.
However, when these 
tools cannot prove that a given function satisfies a specification, to discover the error, 
the user needs to go through a complicated proof trace and understand it in detail (\javert), or even act without any proof trace at all (KJS).

We show how \jilette can be used as an auxiliary mechanism for debugging 
separation logic specifications of JavaScript programs used by \javert. \polish{stopped here.} 
...
\jilette is \emph{I want to say it has actual applications}, because something. 

\jilette is \emph{approachable}, in that it can easily be used by JavaScript developers. 


\jilette is \emph{trustworthy}, as its components come with correctness guarantees: the correctness of the \JSComp compiler ensures full adherence to the real semantics of JavaScript, whereas the soundness result for the \jsil symbolic execution in \jilette guarantees absence of false positives.

\pmax{How do we know that we get OUR symbolic execution in Rosette?}

% Switch in C1 for strict mode
% Something in C1 has to connected to JSIL

Few tools. Functional correctness - JaVerT, K, but too many annotations. There are others, this and that. Symbolic execution? Very few have theory or correctness results, they are critical. \jilette is: useful, approachable, and correct.
