...
There are only a few tools (for example, \javert \cite{javert} and KJS \cite{kjs}) that even support such expressivity. They target the specialist developer wanting rich, 
...
into the \polish{first-order assertion language} of Rosette; 
generating symbolic tests from these translated specifications; 
and running the tests using \jilette. 
...
More importantly, \jilette then generates a concrete counter-model that 
...
There are others, this and that. Symbolic execution? Very few have theory or correctness results, they are critical. \jilette is: useful, approachable, and correct.
